...
Namespace Paginas.Comercial
    ...
    Public Class Aniversariantes
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("NOME")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("NOME")
                        End If

                        If IsDBNull(drow("CPF")) Then
                            e.Row.Cells(1).Text = ""
                        Else
                            e.Row.Cells(1).Text = drow("CPF")
                        End If

                        If IsDBNull(drow("DATA_NASC")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("DATA_NASC")
                        End If

                        If IsDBNull(drow("EMAIL")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("EMAIL")
                        End If

                        If IsDBNull(drow("CELULAR")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("CELULAR")
                        End If

                        If IsDBNull(drow("SITUACAO")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("SITUACAO")
                        End If

                        If IsDBNull(drow("IND_ATRASO")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("IND_ATRASO")
                        End If

                        If IsDBNull(drow("QTD_ATRASO")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("QTD_ATRASO")
                        End If

                        If IsDBNull(drow("REPAC")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("REPAC")
                        End If

                        If IsDBNull(drow("RENEG")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("RENEG")
                        End If

                        If IsDBNull(drow("PREJU")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("PREJU")
                        End If
                        ...
                        If IsDBNull(drow("BNDU")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("BNDU")
                        End If

                        If IsDBNull(drow("PCT_QUITACAO")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = CNumero.FormataNumero(drow("PCT_QUITACAO"), 2)
                        End If

                        If IsDBNull(drow("MAX_ATRASO")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = drow("MAX_ATRASO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_ANIVERSARIANTES '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "'", "SCR_ANIVERSARIANTES", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Aniversariantes_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Aniversariantes " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " </p> ")
