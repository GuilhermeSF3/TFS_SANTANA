...
Namespace Paginas.Comercial
    ...
    Public Class SegundaGarantia
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("OPCODORG3")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("OPCODORG3")
                        End If

                        If IsDBNull(drow("O3DESCR")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("O3DESCR")
                        End If

                        If IsDBNull(drow("ABNRGAR")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("ABNRGAR")
                        End If

                        If IsDBNull(drow("ABMODELO")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("ABMODELO")
                        End If

                        If IsDBNull(drow("ABANOFAB")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("ABANOFAB")
                        End If

                        If IsDBNull(drow("ABANOMOD")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("ABANOMOD")
                        End If

                        If IsDBNull(drow("ABCOR")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("ABCOR")
                        End If

                        If IsDBNull(drow("ABCHASSI")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("ABCHASSI")
                        End If

                        If IsDBNull(drow("OPDTBASE")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("OPDTBASE")
                        End If

                        If IsDBNull(drow("AVENCER")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("AVENCER")
                        End If

                        If IsDBNull(drow("ATRASADAS")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("ATRASADAS")
                        End If

                        If IsDBNull(drow("NAOPAGAS")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = drow("NAOPAGAS")
                        End If

                        If IsDBNull(drow("QTDE")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = drow("QTDE")
                        End If

                        If IsDBNull(drow("OPVLROPCZ")) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = CNumero.FormataNumero(drow("OPVLROPCZ"), 2)
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_SEGUNDA_GARANTIA] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 4, 2) & Left(txtDataDe.Text, 2) &
                                                                "', '" & Right(txtDataAte.Text, 4) & Mid(txtDataAte.Text, 4, 2) & Left(txtDataAte.Text, 2) &
                                                                "'", "SEGUNDAGARANTIA", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("SegundaGarantia{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Segunda Garantia - De: " & txtDataDe.Text & " até: " & txtDataAte.Text)
