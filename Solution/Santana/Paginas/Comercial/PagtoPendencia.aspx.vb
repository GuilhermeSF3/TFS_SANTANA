...
Namespace Paginas.Comercial
    ...
    Public Class PagtoPendencia
        ...
        Private Sub Carrega_Agente()
            Try
                ...
                Else
                    ...
                    'Dim cmd As New SqlCommand("Select top 1 O3DESCR from CDCSANTANAMicroCredito..TORG3 (nolock) where O3codorg in (" & codGerente & ")", con)
                ...
                ElseIf e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("CONTRATO")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("CONTRATO")
                        End If
                        ...
                        If IsDBNull(drow("CLIENTE")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("CLIENTE")
                        End If

                        If IsDBNull(drow("DT_INT")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("DT_INT")
                        End If

                        If IsDBNull(drow("MOTIVO")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("MOTIVO")
                        End If
                        ...
                        If IsDBNull(drow("COD_LOJA")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("COD_LOJA")
                        End If

                        If IsDBNull(drow("LOJA")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("LOJA")
                        End If

                        If IsDBNull(drow("COD_OPERADOR")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("COD_OPERADOR")
                        End If

                        If IsDBNull(drow("OPERADOR")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("OPERADOR")
                        End If

                        If IsDBNull(drow("ANALISTA")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("ANALISTA")
                        End If

                        If IsDBNull(drow("SITUACAO")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = drow("SITUACAO")
                        End If

                        If IsDBNull(drow("PRAZO")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = drow("PRAZO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("SCR_PEND_PAGTO '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', " &
                                                             ddlAgente.SelectedValue, "SCR_PEND_PAGTO", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("PendenciaPagto_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Pendências de Pagamento - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")
