...
Namespace Paginas.Comercial
    ...
    Public Class PendenciasFormalizacao
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("COD_REGIAO")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("COD_REGIAO")
                        End If
                        If IsDBNull(drow("REGIAO")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("REGIAO")
                        End If

                        If IsDBNull(drow("A13CODORG")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("A13CODORG")
                        End If

                        If IsDBNull(drow("A13DESCR")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("A13DESCR")
                        End If


                        If IsDBNull(drow("INTEGRADAS")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("INTEGRADAS")
                        End If

                        If IsDBNull(drow("PENDENCIAS")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("PENDENCIAS")
                        End If

                        If IsDBNull(drow("PRC")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = CNumero.FormataNumero(drow("PRC"), 2)
                        End If

                        e.Row.Cells(9).Text = drow("PEND_AGENTE")
                        e.Row.Cells(10).Text = CNumero.FormataNumero(drow("PRC_PEND_AGENTE"), 2)
                        e.Row.Cells(11).Text = drow("PEND_CONTRATO")
                        e.Row.Cells(12).Text = CNumero.FormataNumero(drow("PRC_PEND_CONTRATO"), 2)
                        e.Row.Cells(13).Text = drow("PEND_GRAVAMES")
                        e.Row.Cells(14).Text = CNumero.FormataNumero(drow("PRC_PEND_GRAVAMES"), 2)
                        e.Row.Cells(15).Text = drow("QTD_PG_CTA_AG")
                        e.Row.Cells(16).Text = drow("QTD_BALCAO")
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_PendFORMALIZ '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', " &
                                                             ddlAgente.SelectedValue, "SCR_PendenciaFormalizacao", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("PendenciasFormalizacao_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Pendências de Formalização - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")
