...
Namespace Paginas.Comercial
    ...
    Public Class PropostasBalcao
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("COD_AGENTE")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("COD_AGENTE")
                        End If
                        ...
                        If IsDBNull(drow("VALOR_INTEGRADA")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("VALOR_INTEGRADA")
                        End If

                        If IsDBNull(drow("VALOR_BALCAO_INTEGRADA")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("VALOR_BALCAO_INTEGRADA")
                        End If

                        If IsDBNull(drow("PCT_VALOR")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("PCT_VALOR") & "%"
                        End If

                        If IsDBNull(drow("QTD_INTEGRADA")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("QTD_INTEGRADA")
                        End If

                        If IsDBNull(drow("QTD_BALCAO_INTEGRADA")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("QTD_BALCAO_INTEGRADA")
                        End If

                        If IsDBNull(drow("PCT_QTD")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("PCT_QTD") & "%"
                        End If
                    End If
                End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_PROD_BALCAO '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', '" &
                                                             ddlAgente.SelectedValue & "'", "PropostasBalcao", strConn)
        ...
        Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Propostas Balcão' ,'Propostas Balcão, Contratos de Veículos com pendencias de Regularização de Documentos (codigo da Pendencia 151).');", True)
        End Sub
        ...
        Private Sub Carrega_Agentes()
            ...
            Try
                ...
                "SELECT COD_AGENTE,AGENTE FROM AUX_PROD_BALCAO ORDER BY AGENTE", connection)
                ...
                ddlAgente.DataValueField = "COD_AGENTE"
                ddlAgente.DataTextField = "AGENTE"
                ddlAgente.DataBind()
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Propostas_Balcao_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Propostas Balcão - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")
