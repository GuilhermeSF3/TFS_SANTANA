...
Namespace Paginas.Comercial
    ...
    Public Class PontecialMktShare
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ...
                Dim Ultimo2MesAnterior As Date = Convert.ToDateTime("01/" + Now.Date.AddMonths(-4).ToString("MM/yyyy"))
                txtDataDe.Text = Mid(Convert.ToString(Ultimo2MesAnterior), 4, 7)

                Dim UltimoMesAnterior As Date = Convert.ToDateTime("01/" + Now.Date.AddMonths(-2).ToString("MM/yyyy"))
                txtDataATE.Text = Mid(Convert.ToString(UltimoMesAnterior), 4, 7)
        ...
        Private Sub Carrega_COMBO()
            ...
            Dim Todos = New ListItem
            Todos.Value = "0"
            Todos.Text = "(TODOS)"

            Dim objDataRegiao = New DbRegiao
            objDataRegiao.CarregarTodosRegistrosList(lbREGIAO)
            lbREGIAO.Items.Add(Todos)

            Dim objDataUF = New DbUF
            objDataUF.CarregarTodosRegistros(ddlUF)
            ddlUF.Items.Add(Todos)
            SortDropDown(ddlUF)
            ...
            If codGerente = "99" Then
                ...
                ddlAGENTE.Items.Add(Todos)
                SortDropDown(ddlAGENTE)
                ddlAGENTE.ClearSelection()
            Else
                Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
                Dim con As New SqlConnection(strConn)
                Dim Vagente As String = ""
                Dim cmd As New SqlCommand(
                            "Select O3DESCR, O3CODORG from CDCSANTANAMicroCredito..TORG3 (nolock) where O3codorg IN (" & codGerente & ")", con)
                cmd.Connection.Open()
            ...
            Dim objDataCidade = New DbCidade
            objDataCidade.CarregarTodosRegistrosDDL(ddlCidade)
            ddlCidade.Items.Add(Todos)
            SortDropDown(ddlCidade)
        ...
        Protected Sub btnDataAnterior1_Click(sender As Object, e As EventArgs)
            ...
            Dim UltimoDiaMesAnterior As Date = Convert.ToDateTime("01/" + txtDataDe.Text)
            ...
            If UltimoDiaMesAnterior.Year = Now.Date.Year Then
                If UltimoDiaMesAnterior.Month <= Now.Date.Month Then
                    txtDataDe.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
                End If
            ElseIf UltimoDiaMesAnterior.Year < Now.Date.Year Then
                txtDataDe.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
            End If
            txtDataDe.Text = Mid(UltimoDiaMesAnterior, 4, 8)
        End Sub
        ...
        Protected Sub btnDataAnterior2_Click(sender As Object, e As EventArgs)
            ...
            Dim UltimoDiaMesAnterior As Date = Convert.ToDateTime("01/" + txtDataATE.Text)
            UltimoDiaMesAnterior = UltimoDiaMesAnterior.AddMonths(-1)

            If UltimoDiaMesAnterior.Year = Now.Date.Year Then
                If UltimoDiaMesAnterior.Month <= Now.Date.Month Then
                    txtDataATE.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
                End If
            ElseIf UltimoDiaMesAnterior.Year < Now.Date.Year Then
                txtDataATE.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
            End If
            txtDataATE.Text = Mid(UltimoDiaMesAnterior, 4, 8)
        End Sub
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        e.Row.Cells(0).Text = drow("UF")
                        e.Row.Cells(1).Text = drow("REGIAO")
                        e.Row.Cells(2).Text = drow("GRUPO")
                        e.Row.Cells(3).Text = drow("CIDADE")
                        e.Row.Cells(4).Text = drow("AGENTE")
                        e.Row.Cells(5).Text = drow("CEP_DE")
                        e.Row.Cells(6).Text = drow("CEP_ATE")

                        e.Row.Cells(7).Text = drow("FX_ANO")

                        e.Row.Cells(8).Text = drow("FX1_QTDE")
                        e.Row.Cells(9).Text = CNumero.FormataNumero(drow("FX1_VLR"), 2)
                        e.Row.Cells(10).Text = CNumero.FormataNumero(drow("FX1_PRC"), 2)

                        e.Row.Cells(11).Text = drow("FX2_QTDE")
                        e.Row.Cells(12).Text = CNumero.FormataNumero(drow("FX2_VLR"), 2)
                        e.Row.Cells(13).Text = CNumero.FormataNumero(drow("FX2_MKT"), 2)
                        e.Row.Cells(14).Text = CNumero.FormataNumero(drow("FX2_PRC"), 2)

                        e.Row.Cells(15).Text = CNumero.FormataNumero(drow("FX1_MKT"), 2)
        ...
        Private Function GetData() As DataTable
            ...
            Dim param_agente As String
            If ddlAGENTE.SelectedValue = "0" Then
                param_agente = ddlAGENTE.SelectedValue + "-" + ddlAGENTE.SelectedItem.Text
            Else
                param_agente = ddlAGENTE.SelectedValue.PadLeft(6, "0") + "-" + ddlAGENTE.SelectedItem.Text
            End If
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_POT_MKT_SHARE] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 1, 2) & "', '" & Right(txtDataATE.Text, 4) & Mid(txtDataATE.Text, 1, 2) & "', '" & txtLocalizar.Text & "', '" & txtCepDe.Text & "', '" & TxtCepAte.Text & "', '" & lbREGIAO.SelectedItem.Text & "', " & ddlUF.SelectedValue & ", '" & param_agente & "', '" & ddlFxAno.SelectedValue & "', '" & ddlCidade.SelectedItem.Text & "','99'", "PMS", strConn)
        ...
        Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Potencial Market Share' ,'Cruza os dados do Potencial Analítico e Sintético com a base da Santana. Soma os valores de Produção Santana e do Mercado no período de referencia e calcula o Market Share em %.');", True)
        End Sub
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Potencial_MarketShare_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Datas de: " + txtDataDe.Text + " até: " + txtDataATE.Text + " Filtro: " + txtLocalizar.Text + " CEP De: " + txtCepDe.Text + " até: " + TxtCepAte.Text + " Região: ")
