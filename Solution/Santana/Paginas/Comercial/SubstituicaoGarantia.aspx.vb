...
Namespace Paginas.Comercial
    ...
    Public Class SubstituicaoGarantia
        ...
        Private Sub Carrega_Agente()
            ...
            Try
                ...
                "SELECT O3CODORG AS AGCod,O3DESCR  + ' - ' + CAST(O3CODORG AS VARCHAR(10)) AS Descricao  from  CDCSANTANAMicroCredito..TORG3 (nolock) ORDER BY O3DESCR", connection)
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("QTD_SUBSTITUI")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = CNumero.FormataNumero(drow("QTD_SUBSTITUI"), 0)
                        End If

                        If IsDBNull(drow("VLR_SUBSTITUI")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = CNumero.FormataNumero(drow("VLR_SUBSTITUI"), 2)
                        End If

                        If IsDBNull(drow("QTD_TRANSFER")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = CNumero.FormataNumero(drow("QTD_TRANSFER"), 0)
                        End If

                        If IsDBNull(drow("VLR_TRANSFER")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = CNumero.FormataNumero(drow("VLR_TRANSFER"), 2)
                        End If

                        If IsDBNull(drow("QTD_TTL")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = CNumero.FormataNumero(drow("QTD_TTL"), 0)
                        End If

                        If IsDBNull(drow("VLR_TTL")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = CNumero.FormataNumero(drow("VLR_TTL"), 2)
                        End If

                        If IsDBNull(drow("OPERACAO")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("OPERACAO")
                        End If

                        If IsDBNull(drow("DATA_OCOR")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("DATA_OCOR")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_SubstituicaoGarantia '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', " &
                                                             ddlAgente.SelectedValue, "SCR_PendenciaContrato", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("SubstituicaoGarantia_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Substituição de Garantia - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")


                        Response.Write("<div><span>RESSARCIMENTO DE TARIFAS AOS AGENTES</span></div>")


                        Response.Output.Write(sw.ToString())


                        Response.Write("<div><span style='color:red'>Na data de hoje o valor para reembolso é de R$ 150,00 por operação.</span></div>")
