...
Namespace Paginas.Comercial
    ...
    Public Class PendenciasContrato
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("OPNRPROP")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("OPNRPROP")
                        End If

                        If IsDBNull(drow("OPDTBASE")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("OPDTBASE")
                        End If
                        ...
                        If IsDBNull(drow("OPCODORG3")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("OPCODORG3")
                        End If

                        If IsDBNull(drow("O3DESCR")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("O3DESCR")
                        End If

                        If IsDBNull(drow("OPCODORG4")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("OPCODORG4")
                        End If

                        If IsDBNull(drow("O4DESCR")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("O4DESCR")
                        End If

                        If IsDBNull(drow("OBS")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("OBS")
                        End If

                        If IsDBNull(drow("VLR_LIB") Or drow("VLR_LIB") = 0) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = CNumero.FormataNumero(drow("VLR_LIB"), 2)
                        End If

                        If IsDBNull(drow("VLR_RETER") Or drow("VLR_RETER") = 0) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = CNumero.FormataNumero(drow("VLR_RETER"), 2)
                        End If

                        If IsDBNull(drow("VLR_PRODUCAO")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = CNumero.FormataNumero(drow("VLR_PRODUCAO"), 2)
                        End If

                        If IsDBNull(drow("PRC_PEND")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = CNumero.FormataNumero(drow("PRC_PEND"), 2)
                        End If

                        If IsDBNull(drow("DT_LIMITE")) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = drow("DT_LIMITE")
                        End If

                        If Now >= drow("DT_BLOQUEIO") Then
                            Cor = Drawing.ColorTranslator.FromOle(&HCC)

                        Else
                            Cor = Drawing.ColorTranslator.FromOle(&H666666)
                        End If
                        e.Row.Cells(16).Text = drow("DT_BLOQUEIO")
                        e.Row.Cells(16).ForeColor = Cor

                        If IsDBNull(drow("DIASATRASO")) Then
                            e.Row.Cells(17).Text = ""
                        Else
                            e.Row.Cells(17).Text = drow("DIASATRASO")
                        End If

                        If IsDBNull(drow("ATRASO_PEN")) Then
                            e.Row.Cells(18).Text = ""
                        Else
                            e.Row.Cells(18).Text = drow("ATRASO_PEN")
                        End If
        ...
        Protected Sub BindGridView1Data()
            ...
            If Not data Is Nothing AndAlso Not data.Rows Is Nothing AndAlso data.Rows.Count > 0 Then
                ...
                msgFooter.Visible = True
            Else
                msgHeader.Visible = False
                msgFooter.Visible = False
            End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim comando As SqlCommand = New SqlCommand(
                "SCR_PendenciaContrato_TTL '" &
                oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                      oDataAte.Data.ToString("yyyyMMdd") & "', " & ddlAgente.SelectedValue & ", '" &
                                                             ddlRegiao.SelectedValue & "', '" &
                                                             ddlPreju.SelectedValue & "', '" &
                                                             ddlReneg.SelectedValue & "', '" &
                                                             ddlAtivo.SelectedValue & "'", conexao)

            comando.Connection.Open()
            Dim ddlValues As SqlDataReader
            ddlValues = comando.ExecuteReader()
            ddlValues.Read()
            txtQteContratoRealizado.Text = CNumero.FormataNumero(ddlValues("QTD").ToString(), 0)
            TxtVlr.Text = CNumero.FormataNumero(ddlValues("VLR").ToString(), 2)
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_PendenciaContrato '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', " &
                                                             ddlAgente.SelectedValue & ", '" &
                                                             ddlRegiao.SelectedValue & "', '" &
                                                             ddlPreju.SelectedValue & "', '" &
                                                             ddlReneg.SelectedValue & "', '" &
                                                             ddlAtivo.SelectedValue & "'", "SCR_PendenciaContrato", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("PendenciasContrato_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Pendências de Contrato - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")

                        Response.Write("Qtde de CONTRATOS " & txtQteContratoRealizado.Text & " Valor Liberado " & TxtVlr.Text)

                        Response.Write("<div><span>O(s) contrato(s) abaixo relacionado(s) estão incompletos, ou seja, Pendentes de Formalização a mais de 10 (dez) dias da data base da operação.</span></div>")
                        Response.Write("<div><span><small>Lembramos que, a não regularização desta(s) pendência(s) no prazo máximo de 10 (dez) dias uteis da data base das respectivas operações, acarretará a retenção das Comissões de Financiamentos  deste(s) contrato(s) no fechamento mensal.</small></span></div> </P>")

                        Response.Output.Write(sw.ToString())

                        Response.Write("</P><div><span style='color:red'>Atenção: O envio do processo completo é determinante para a Formalização e Legalização do Contrato. Portanto a ocorrência de uma destas irregularidades acima é considerada como FALTA DO CONTRATO, ou seja, CONTRATO PENDENTE. (Vide Comunicado 014/2013).</span></div> <P></P>")
