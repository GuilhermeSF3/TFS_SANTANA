...
Namespace Paginas.Comercial
    ...
    Public Class PendenciasCRVsint
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("QTDE")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("QTDE")
                        End If

                        If IsDBNull(drow("QTDE_PEND")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("QTDE_PEND")
                        End If

                        If IsDBNull(drow("PRC")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            'e.Row.Cells(4).Text = drow("PRC")
                            e.Row.Cells(5).Text = If(IsDBNull(drow("PRC")), 0, CNumero.FormataNumero(drow("PRC"), 2))
                        End If

                        If IsDBNull(drow("VLR_RETIDO")) Then
                            e.Row.Cells(6).Text = ""

                        Else
                            'e.Row.Cells(5).Text = drow("VLR_RETIDO")
                            e.Row.Cells(6).Text = If(IsDBNull(drow("VLR_RETIDO")), 0, CNumero.FormataNumero(drow("VLR_RETIDO"), 2))
                            If drow("VLR_RETIDO") = 0.0 Then
                                e.Row.Cells(6).ForeColor = Drawing.ColorTranslator.FromOle(&H0)
                            Else
                                e.Row.Cells(6).ForeColor = Drawing.ColorTranslator.FromOle(255) '&HCC0000
                            End If
                        End If

                        If IsDBNull(drow("PRC_ANT")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            'e.Row.Cells(6).Text = drow("PRC_ANT")
                            e.Row.Cells(7).Text = If(IsDBNull(drow("PRC_ANT")), 0, CNumero.FormataNumero(drow("PRC_ANT"), 2))
                        End If

                        If IsDBNull(drow("QTDE_ANT")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("QTDE_ANT")
                        End If

                        If IsDBNull(drow("QTDE_PEND_ANT")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("QTDE_PEND_ANT")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("SCR_CRV_sintetico '" & oDataContrato.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataReferencia.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', " &
                                                             ddlAgente.SelectedValue, "SCR_CRV", strConn)
