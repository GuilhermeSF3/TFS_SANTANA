...
Namespace Paginas.Comercial
    ...
    Public Class AcionamentoCliente
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("DT_BASE")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("DT_BASE")
                        End If
                        ...
                        If IsDBNull(drow("BAIRRO")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("BAIRRO")
                        End If
                        ...
                        If IsDBNull(drow("TELFIXO")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("TELFIXO")
                        End If

                        If IsDBNull(drow("CELULAR")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("CELULAR")
                        End If

                        If IsDBNull(drow("FONE_COM")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("FONE_COM")
                        End If

                        If IsDBNull(drow("PROFISSAO")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("PROFISSAO")
                        End If
                        ...
                        If IsDBNull(drow("PARCELAS_PAGAS")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = drow("PARCELAS_PAGAS")
                        End If

                        If IsDBNull(drow("DT_ULT_PGTO")) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = drow("DT_ULT_PGTO")
                        End If

                        If IsDBNull(drow("RENDA")) Then
                            e.Row.Cells(16).Text = ""
                        Else
                            e.Row.Cells(16).Text = drow("RENDA")
                        End If

                        If IsDBNull(drow("OCUPACAO")) Then
                            e.Row.Cells(17).Text = ""
                        Else
                            e.Row.Cells(17).Text = drow("OCUPACAO")
                        End If

                        If IsDBNull(drow("VALOR_LIB")) Then
                            e.Row.Cells(18).Text = ""
                        Else
                            e.Row.Cells(18).Text = CNumero.FormataNumero(drow("VALOR_LIB"), 2)
                        End If

                        If IsDBNull(drow("VALOR_FIN")) Then
                            e.Row.Cells(19).Text = ""
                        Else
                            e.Row.Cells(19).Text = CNumero.FormataNumero(drow("VALOR_FIN"), 2)
                        End If

                        If IsDBNull(drow("COD_PROD")) Then
                            e.Row.Cells(20).Text = ""
                        Else
                            e.Row.Cells(20).Text = drow("COD_PROD")
                        End If

                        If IsDBNull(drow("PRODUTO")) Then
                            e.Row.Cells(21).Text = ""
                        Else
                            e.Row.Cells(21).Text = drow("PRODUTO")
                        End If

                        If IsDBNull(drow("DATA_NASCTO")) Then
                            e.Row.Cells(22).Text = ""
                        Else
                            e.Row.Cells(22).Text = drow("DATA_NASCTO")
                        End If

                        If IsDBNull(drow("COD_AGENTE")) Then
                            e.Row.Cells(23).Text = ""
                        Else
                            e.Row.Cells(23).Text = drow("COD_AGENTE")
                        End If

                        If IsDBNull(drow("AGENTE")) Then
                            e.Row.Cells(24).Text = ""
                        Else
                            e.Row.Cells(24).Text = drow("AGENTE")
                        End If

                        If IsDBNull(drow("ATRASO")) Then
                            e.Row.Cells(25).Text = ""
                        Else
                            e.Row.Cells(25).Text = drow("ATRASO")
                        End If

                        If IsDBNull(drow("SCORE")) Then
                            e.Row.Cells(26).Text = ""
                        Else
                            e.Row.Cells(26).Text = drow("SCORE")
                        End If

                        If IsDBNull(drow("RENEG")) Then
                            e.Row.Cells(27).Text = ""
                        Else
                            e.Row.Cells(27).Text = drow("RENEG")
                        End If

                        If IsDBNull(drow("CLI_SANTANA")) Then
                            e.Row.Cells(28).Text = ""
                        Else
                            e.Row.Cells(28).Text = drow("CLI_SANTANA")
                        End If

                        If IsDBNull(drow("DT_LIQ")) Then
                            e.Row.Cells(29).Text = ""
                        Else
                            e.Row.Cells(29).Text = drow("DT_LIQ")
                        End If

                        If IsDBNull(drow("ATRASO")) Then
                            e.Row.Cells(30).Text = ""
                        Else
                            e.Row.Cells(30).Text = drow("ATRASO")
                        End If
                        ...
                        If IsDBNull(drow("VLR_FIPE")) Then
                            e.Row.Cells(33).Text = ""
                        Else
                            e.Row.Cells(33).Text = CNumero.FormataNumero(drow("VLR_FIPE"), 2)
                        End If

                        If IsDBNull(drow("EMAIL")) Then
                            e.Row.Cells(34).Text = ""
                        Else
                            e.Row.Cells(34).Text = drow("EMAIL")
                        End If

                        If IsDBNull(drow("SALDO_DEVEDOR")) Then
                            e.Row.Cells(35).Text = ""
                        Else
                            e.Row.Cells(35).Text = CNumero.FormataNumero(drow("SALDO_DEVEDOR"), 2)
                        End If

                        If IsDBNull(drow("ANO_FABRIC")) Then
                            e.Row.Cells(36).Text = ""
                        Else
                            e.Row.Cells(36).Text = drow("ANO_FABRIC")
                        End If

                        If IsDBNull(drow("ANO_MODELO")) Then
                            e.Row.Cells(37).Text = ""
                        Else
                            e.Row.Cells(37).Text = drow("ANO_MODELO")
                        End If

                        If IsDBNull(drow("PREJUIZO")) Then
                            e.Row.Cells(38).Text = ""
                        Else
                            e.Row.Cells(38).Text = drow("PREJUIZO")
                        End If

                        If IsDBNull(drow("BNDU")) Then
                            e.Row.Cells(39).Text = ""
                        Else
                            e.Row.Cells(39).Text = drow("BNDU")
                        End If

                        If IsDBNull(drow("ATRASO_5D")) Then
                            e.Row.Cells(40).Text = ""
                        Else
                            e.Row.Cells(40).Text = drow("ATRASO_5D")
                        End If

                        If IsDBNull(drow("REPAC")) Then
                            e.Row.Cells(41).Text = ""
                        Else
                            e.Row.Cells(41).Text = drow("REPAC")
                        End If

                        If IsDBNull(drow("PRC_LIQ")) Then
                            e.Row.Cells(42).Text = ""
                        Else
                            e.Row.Cells(42).Text = CNumero.FormataNumero(drow("PRC_LIQ"), 2)
                        End If

                        If IsDBNull(drow("ANTECIPACAO")) Then
                            e.Row.Cells(43).Text = ""
                        Else
                            e.Row.Cells(43).Text = drow("ANTECIPACAO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("SCR_ACIONAMENTO_CLIENTE '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" &
                                                             oDataAte.Data.ToString("yyyyMMdd") & "', '" &
                                                             ddlAgente.SelectedValue & "'", "SCR_ACIONAMENTO_CLIENTE", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("AcionamentoClientes_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Pendências de Gravames - Data De. " & txtDataDe.Text & " Data Até. " & txtDataAte.Text & " -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")
