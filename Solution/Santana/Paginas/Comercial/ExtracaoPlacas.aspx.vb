...
Namespace Paginas.Credito
    ...
    Public Class ExtracaoPlacas
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("PARC_ABERTO")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("PARC_ABERTO")
                        End If

                        If IsDBNull(drow("OPVLRPARC")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("OPVLRPARC")
                        End If

                        If IsDBNull(drow("ABPLACA")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("ABPLACA")
                        End If

                        If IsDBNull(drow("SITUACAO")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("SITUACAO")
                        End If

                        If IsDBNull(drow("DESCR_TIPO")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("DESCR_TIPO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_EXTRATOR_PLACA] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 4, 2) & Left(txtDataDe.Text, 2) & "', " &
                                                                "'" & ddlProduto.SelectedValue & "'", "EXTRATORPLACA", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Extração_Placas_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Data Ref.: " & txtDataDe.Text & " Veículo: " & ddlProduto.Text)
