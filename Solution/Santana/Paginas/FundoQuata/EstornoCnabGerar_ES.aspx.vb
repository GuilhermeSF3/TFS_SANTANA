...
Namespace Paginas.FundoQuata
    ...
    Public Class EstornoCnabGerar_ES
        ...
        Private Sub GravarLogExecucao(usuario As String, dataReferencia As String)
            ...
            Dim acao As String = "Estorno - Cnab - Gerar"
        ...
        Private Sub GravarLogExecucaoReneg(usuario As String, dataReferencia As String)
            ...
            Try
                Using con As New SqlConnection(strConn)
                    Using cmd As New SqlCommand("INSERT INTO LogCnabReneg (Usuario, DataReferencia, DataExecucao, Observacao, Acao) VALUES (@Usuario, @DataReferencia, @DataExecucao, @Observacao, @Acao)", con)
                        cmd.Parameters.AddWithValue("@Usuario", usuario)
                        cmd.Parameters.AddWithValue("@DataReferencia", dataReferencia)
                        cmd.Parameters.AddWithValue("@DataExecucao", DateTime.Now)
                        cmd.Parameters.AddWithValue("@Observacao", observ)
        ...
        Private Function GetData(dataReferencia As String) As DataTable
            ...
            Using con As New SqlConnection(strConn)
                ...
                End Using
            End Using
            If resultTable.Rows.Count > 0 AndAlso resultTable.Rows(0)(0).ToString().Contains("Sem movimentação para gerar o CNAB550 - ESTORNO") Then

                ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "alert", "alert('Sem movimentação para gerar o CNAB550 - ESTORNO');", True)

                Return Nothing
                GravarLogExecucaoReneg(usuarioLogado, dataReferencia)
            End If
            Return resultTable
        End Function
        ...
        Private Sub ExportarParaTXT(dt As DataTable)
            Try
                Dim nomeArquivo As String = "CB" & DateTime.Now.ToString("ddMM") & "01" & "_ES" & ".rem"
                Response.Clear()
