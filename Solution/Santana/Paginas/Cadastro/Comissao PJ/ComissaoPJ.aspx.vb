       ...
       Private Sub Carrega_Operador()
        ...
        Private Sub Carrega_Relatorio()
            ...
            Try
                ...
                ddlRelatorio.Items.Insert(0, New ListItem("Resumo por Produto", "1"))
                ddlRelatorio.Items.Insert(1, New ListItem("Resumo - Grupo de Operadores", "2"))
                ddlRelatorio.Items.Insert(2, New ListItem("Por Operadores", "3"))
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("modalidade")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("modalidade")
                        End If

                        If IsDBNull(drow("cod_modalidade")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("cod_modalidade")
                        End If

                         If IsDBNull(drow("Tx_Efetiva") Or drow("Tx_Efetiva") = 0) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = CNumero.FormataNumero(drow("Tx_Efetiva"), 2)
                        End If

                         If IsDBNull(drow("Tx_Ponderada") Or drow("Tx_Ponderada") = 0) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = CNumero.FormataNumero(drow("Tx_Ponderada"), 2)
                        End If

                         If IsDBNull(drow("PV") Or drow("PV") = 0) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = CNumero.FormataNumero(drow("PV"), 2)
                        End If

                         If IsDBNull(drow("FV") Or drow("FV") = 0) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = CNumero.FormataNumero(drow("FV"), 2)
                        End If

                         If IsDBNull(drow("Vlr_Liberado") Or drow("Vlr_Liberado") = 0) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = CNumero.FormataNumero(drow("Vlr_Liberado"), 2)
                        End If

                         If IsDBNull(drow("Prz_Medio") Or drow("Prz_Medio") = 0) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = CNumero.FormataNumero(drow("Prz_Medio"), 2)
                        End If

                         If IsDBNull(drow("Qtd") Or drow("Qtd") = 0) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = CNumero.FormataNumero(drow("Qtd"), 0)
                        End If

                         If IsDBNull(drow("prc_comissao") Or drow("prc_comissao") = 0) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = CNumero.FormataNumero(drow("prc_comissao"), 2)
                        End If

                         If IsDBNull(drow("vlr_Comissao") Or drow("vlr_Comissao") = 0) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = CNumero.FormataNumero(drow("vlr_Comissao"), 2)
                        End If

                         If IsDBNull(drow("Tac") Or drow("Tac") = 0) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = CNumero.FormataNumero(drow("Tac"), 2)
                        End If

                         If IsDBNull(drow("DESCONTO") Or drow("DESCONTO") = 0) Then
                            e.Row.Cells(16).Text = ""
                        Else
                            e.Row.Cells(16).Text = CNumero.FormataNumero(drow("DESCONTO"), 2)
                        End If

                         If IsDBNull(drow("Total1") Or drow("Total1") = 0) Then
                            e.Row.Cells(17).Text = ""
                        Else
                            e.Row.Cells(17).Text = CNumero.FormataNumero(drow("Total1"), 2)
                        End If

                         If IsDBNull(drow("Fixo") Or drow("Fixo") = 0) Then
                            e.Row.Cells(18).Text = ""
                        Else
                            e.Row.Cells(18).Text = CNumero.FormataNumero(drow("Fixo"), 2)
                        End If

                         If IsDBNull(drow("comissao_Total") Or drow("comissao_Total") = 0) Then
                            e.Row.Cells(19).Text = ""
                        Else
                            e.Row.Cells(19).Text = CNumero.FormataNumero(drow("comissao_Total"), 2)
                        End If

                         If IsDBNull(drow("prc_comissao2") Or drow("prc_comissao2") = 0) Then
                            e.Row.Cells(20).Text = ""
                        Else
                            e.Row.Cells(20).Text = CNumero.FormataNumero(drow("prc_comissao2"), 2)
                        End If

                         If IsDBNull(drow("Meta") Or drow("Meta") = 0) Then
                            e.Row.Cells(21).Text = ""
                        Else
                            e.Row.Cells(21).Text = CNumero.FormataNumero(drow("Meta"), 2)
                        End If

                         If IsDBNull(drow("prc_Meta") Or drow("prc_Meta") = 0) Then
                            e.Row.Cells(22).Text = ""
                        Else
                            e.Row.Cells(22).Text = CNumero.FormataNumero(drow("prc_Meta"), 2)
                        End If
                     
                        If IsDBNull(drow("Cumpriu_Meta")) Then
                            e.Row.Cells(23).Text = ""
                        Else
                            e.Row.Cells(23).Text = drow("Cumpriu_Meta")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("[SCR_PJ_COMISSAO_OPERADORES] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', '" & ddlRelatorio.SelectedValue & "','" & ddlOperador.SelectedValue & "'", "ComissaoPj", strConn)
        ...
        Protected Sub btnCarregar_Click(sender As Object, e As EventArgs)
            Try
                ...
                lblRelatorio.Text = String.Format("Comissão PJ - {0}", ddlRelatorio.SelectedItem.Text)
                BindGridView1Data()
        ...
        Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
            ...
            Try
                ...
                If Not IsNothing(GridView1.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("ComissaoPJ_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write(String.Format("Comissão PJ - {0}", ddlRelatorio.SelectedItem.Text))
