    ...
    Public Class Tabelas
        ...
        Private Sub Carrega_Produtos()
            ...
            Try
                ...
                "SELECT TPCODPRD,TPCODPRD + ' - ' + TPDESCR AS TPDESCR FROM CDCSANTANAMICROCREDITO..TPROD (NOLOCK) ORDER BY TPDESCR", connection)
                ...
                ddlProduto.DataValueField = "TPCODPRD"
                ddlProduto.DataTextField = "TPDESCR"
                ddlProduto.DataBind()
            ...
            Try
                ...
                ElseIf e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("CÓDIGO TABELA")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("CÓDIGO TABELA")
                        End If

                        If IsDBNull(drow("DESCRIÇÃO TABELA")) Then
                            e.Row.Cells(1).Text = ""
                        Else
                            e.Row.Cells(1).Text = drow("DESCRIÇÃO TABELA")
                        End If

                        If IsDBNull(drow("CÓDIGO PRODUTO")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("CÓDIGO PRODUTO")
                        End If

                        If IsDBNull(drow("DESCRIÇÃO PRODUTO")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("DESCRIÇÃO PRODUTO")
                        End If

                        If IsDBNull(drow("DATA INÍCIO")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("DATA INÍCIO")
                        End If

                        If IsDBNull(drow("DATA FIM")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("DATA FIM")
                        End If

                        If IsDBNull(drow("SITUAÇÃO")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("SITUAÇÃO")
                        End If

                        If IsDBNull(drow("ENTRADA MINIMA")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("ENTRADA MINIMA")
                        End If

                        If IsDBNull(drow("SCORE")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("SCORE")
                        End If

                        If IsDBNull(drow("CÓDIGO DA OCUPAÇÃO")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("CÓDIGO DA OCUPAÇÃO")
                        End If

                        If IsDBNull(drow("POLÍTICA DESPESA")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("POLÍTICA DESPESA")
                        End If

                        If IsDBNull(drow("RETORNO")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("RETORNO")
                        End If

                        If IsDBNull(drow("CADASTRO")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("CADASTRO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("[SCR_TABELAS] '" & ddlRelatorio.SelectedValue & "', '" & ddlProduto.SelectedValue & "'", "Tabelas", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Tabelas_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Select Case ddlRelatorio.SelectedValue
                            Case 1
                                Response.Write(String.Format("RELAÇÃO GERAL DE TABELAS"))
                            Case 2
                                Response.Write(String.Format("RELAÇÃO TABELAS ATIVAS"))
                            Case 3
                                Response.Write(String.Format("RELAÇÃO DE TABELAS INATIVAS"))
                        End Select
                        Response.Write("<p>")
