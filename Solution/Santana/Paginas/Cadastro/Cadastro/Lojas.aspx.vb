...
Namespace Paginas.Cadastro
    ...
    Public Class Lojas
            ...
            Try
                ...
                ElseIf e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("CÓDIGO LOJA")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("CÓDIGO LOJA")
                        End If

                        If IsDBNull(drow("NOME LOJA")) Then
                            e.Row.Cells(1).Text = ""
                        Else
                            e.Row.Cells(1).Text = drow("NOME LOJA")
                        End If

                        If IsDBNull(drow("DESCRIÇÃO LOJA")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("DESCRIÇÃO LOJA")
                        End If

                        If IsDBNull(drow("ENDEREÇO")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("ENDEREÇO")
                        End If

                        If IsDBNull(drow("COMPLEMENTO")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("COMPLEMENTO")
                        End If

                        If IsDBNull(drow("CEP")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("CEP")
                        End If
                        ...
                        If IsDBNull(drow("TELEFONE")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("TELEFONE")
                        End If

                        If IsDBNull(drow("CPF/CNPJ")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("CPF/CNPJ")
                        End If

                        If IsDBNull(drow("LOJA ATIVA")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("LOJA ATIVA")
                        End If

                        If IsDBNull(drow("DATA DESATIVAÇÃO")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("DATA DESATIVAÇÃO")
                        End If

                        If IsDBNull(drow("DATA ATIVAÇÃO")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = drow("DATA ATIVAÇÃO")
                        End If

                        If IsDBNull(drow("DATA CADASTRO LOJA")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = drow("DATA CADASTRO LOJA")
                        End If

                        If IsDBNull(drow("CÓDIGO AGENTE")) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = drow("CÓDIGO AGENTE")
                        End If

                        If IsDBNull(drow("DESCRIÇÃO AGENTE")) Then
                            e.Row.Cells(16).Text = ""
                        Else
                            e.Row.Cells(16).Text = drow("DESCRIÇÃO AGENTE")
                        End If

                        If IsDBNull(drow("BANCO 1")) Then
                            e.Row.Cells(17).Text = ""
                        Else
                            e.Row.Cells(17).Text = drow("BANCO 1")
                        End If

                        If IsDBNull(drow("AGENCIA 1")) Then
                            e.Row.Cells(18).Text = ""
                        Else
                            e.Row.Cells(18).Text = drow("AGENCIA 1")
                        End If

                        If IsDBNull(drow("CONTA 1")) Then
                            e.Row.Cells(19).Text = ""
                        Else
                            e.Row.Cells(19).Text = drow("CONTA 1")
                        End If

                        If IsDBNull(drow("BANCO 2")) Then
                            e.Row.Cells(20).Text = ""
                        Else
                            e.Row.Cells(20).Text = drow("BANCO 2")
                        End If

                        If IsDBNull(drow("AGENCIA 2")) Then
                            e.Row.Cells(21).Text = ""
                        Else
                            e.Row.Cells(21).Text = drow("AGENCIA 2")
                        End If

                        If IsDBNull(drow("CONTA 2")) Then
                            e.Row.Cells(22).Text = ""
                        Else
                            e.Row.Cells(22).Text = drow("CONTA 2")
                        End If

                        If IsDBNull(drow("BANCO 3")) Then
                            e.Row.Cells(23).Text = ""
                        Else
                            e.Row.Cells(23).Text = drow("BANCO 3")
                        End If

                        If IsDBNull(drow("AGENCIA 3")) Then
                            e.Row.Cells(24).Text = ""
                        Else
                            e.Row.Cells(24).Text = drow("AGENCIA 3")
                        End If

                        If IsDBNull(drow("CONTA 3")) Then
                            e.Row.Cells(25).Text = ""
                        Else
                            e.Row.Cells(25).Text = drow("CONTA 3")
                        End If
                        ...
                        If IsDBNull(drow("USNOMEUSU")) Then
                            e.Row.Cells(26).Text = ""
                        Else
                            e.Row.Cells(26).Text = drow("USNOMEUSU")
                        End If

                        If IsDBNull(drow("VLROPCZ")) Then
                            e.Row.Cells(27).Text = ""
                        Else
                            e.Row.Cells(27).Text = CNumero.FormataNumero(drow("VLROPCZ"), 2)
                        End If

                        If IsDBNull(drow("EMAIL")) Then
                            e.Row.Cells(28).Text = ""
                        Else
                            e.Row.Cells(28).Text = drow("EMAIL")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("[SCR_LOJAS_OPERADORES] '" & ddlRelatorio.SelectedValue & "', '" & IIf(ddlAgente.SelectedValue <> "99", ddlAgente.SelectedValue.PadLeft(6, "0"), ddlAgente.SelectedValue) & "'", "Lojas", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Lojas{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Select Case ddlRelatorio.SelectedValue
                            Case 1
                                Response.Write(String.Format("RELAÇÃO GERAL DE LOJAS"))
                            Case 2
                                Response.Write(String.Format("RELAÇÃO DE LOJAS ATIVAS"))
                            Case 3
                                Response.Write(String.Format("RELAÇÃO DE LOJAS INATIVAS"))
                        End Select
                        Response.Write("<p>")
