    ...
    Public Class Balancete
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ...
                If ContextoWeb.DadosTransferencia.CodEmpresa <> 0 Then
                    BindGridView1Data()
                    ContextoWeb.DadosTransferencia.CodEmpresa = 0
                End If
        ...
        Private Sub Carrega_Empresa()
            ...
            Try
                ...
                '               "SELECT CODX_EMPR AS COD_EMP, NOME_EMPR AS DESCR  FROM [SRV-AWS].CONTABIL_SANTANA.dbo.EMPRESA", connection)

                ' homologacao
                '                Dim command As SqlCommand = New SqlCommand( _
                '               "SELECT CODX_EMPR AS COD_EMP, NOME_EMPR AS DESCR  FROM CONTABIL_SANTANA..EMPRESA (NOLOCK)", connection)
                ' Producao
                Dim command As SqlCommand = New SqlCommand(
               "SELECT CODX_EMPR AS COD_EMP, NOME_EMPR AS DESCR  FROM [SRV-AWS].ZAP_CNT_P.dbo.EMPRESA ", connection)
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("EMPRESA")) Then
                            e.Row.Cells(1).Text = ""
                        Else
                            e.Row.Cells(1).Text = drow("EMPRESA")
                        End If

                        If IsDBNull(drow("CONTA")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("CONTA")
                        End If

                        If IsDBNull(drow("CONTA_FMT")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("CONTA_FMT")
                        End If
                        ...
                        If IsDBNull(drow("SLD_ANT") Or drow("SLD_ANT") = 0) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = CNumero.FormataNumero(drow("SLD_ANT"), 2)
                        End If

                        If IsDBNull(drow("MOV_DEB") Or drow("MOV_DEB") = 0) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = CNumero.FormataNumero(drow("MOV_DEB"), 2)
                        End If

                        If IsDBNull(drow("MOV_CRED") Or drow("MOV_CRED") = 0) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = CNumero.FormataNumero(drow("MOV_CRED"), 2)
                        End If

                        If IsDBNull(drow("MOV_D_C") Or drow("MOV_D_C") = 0) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = CNumero.FormataNumero(drow("MOV_D_C"), 2)
                        End If

                        If IsDBNull(drow("SLD_ATU") Or drow("SLD_ATU") = 0) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = CNumero.FormataNumero(drow("SLD_ATU"), 2)
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("[SCR_GERENCIAL_BALANCETE] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', '" & ddlEmpresa.SelectedValue & "'", "Balancete", strConn)
        ...
        Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Balancete importado do Contábil' ,'Balancete mensal na data de referencia. Saldos de despesas não zeram em Junho.');", True)
        End Sub
        ...
        Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
            ...
            Try
                ...
                If Not IsNothing(GridView1.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Balancete_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
