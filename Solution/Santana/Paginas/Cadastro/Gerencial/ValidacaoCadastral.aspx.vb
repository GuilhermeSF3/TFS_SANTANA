...
Namespace Paginas.Cadastro.Gerencial
    ...
    Public Class ValidacaoCadastral
        ...
        End Sub
        Private Sub Carrega_Sistemas()
            Try

                ddlSistemas.Items.Insert(0, New ListItem("Todos", "todos"))
                ddlSistemas.Items.Insert(1, New ListItem("FUNÇÃO", "funcao"))
                ddlSistemas.Items.Insert(2, New ListItem("NETFACTOR", "netfactor"))
                ddlSistemas.Items.Insert(3, New ListItem("CRK", "crk"))
                ddlSistemas.Items.Insert(4, New ListItem("CFI", "cfi"))
                ddlSistemas.Items.Insert(5, New ListItem("SIG", "sig"))
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("ORIGEM")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("ORIGEM")
                        End If

                        If IsDBNull(drow("CODIGO DO CLIENTE")) Then
                            e.Row.Cells(1).Text = ""
                        Else
                            e.Row.Cells(1).Text = drow("CODIGO DO CLIENTE")
                        End If

                        If IsDBNull(drow("NOME DO CLIENTE")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("NOME DO CLIENTE")
                        End If

                        If IsDBNull(drow("RETORNO")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("RETORNO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            If ddlSistemas.SelectedValue.ToString() = "todos" Then
                table = Util.ClassBD.GetExibirGrid("SELECT ORIGEM, [CODIGO DO CLIENTE], [NOME DO CLIENTE], [RETORNO] FROM [SRV-AWS.SHOPCRED.LOCAL].ADV_EGUARDIAN.DBO.VIEW_VALIDACAO_CADASTRAL WITH (NOLOCK) ORDER BY 1,3,4", "VALIDACAO_CADASTRAL", strConn)
            Else
                table = Util.ClassBD.GetExibirGrid("SELECT ORIGEM, [CODIGO DO CLIENTE], [NOME DO CLIENTE], [RETORNO] FROM [SRV-AWS.SHOPCRED.LOCAL].ADV_EGUARDIAN.DBO.VIEW_VALIDACAO_CADASTRAL WITH (NOLOCK) WHERE ORIGEM = '" & ddlSistemas.SelectedValue.ToString() & "' ORDER BY 1,3,4", "VALIDACAO_CADASTRAL", strConn)
            End If
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("ValidacaoCadastral_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Relatório de Validação Cadastral")
