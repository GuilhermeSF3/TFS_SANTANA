...
Namespace Paginas.RH

    Public Class CPColaboradoresSintetico
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        col += 1
                        If IsDBNull(drow("OPQTDDPARC")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("OPQTDDPARC")
                        End If

                        col += 1
                        If IsDBNull(drow("OPPARCLIQ")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("OPPARCLIQ")
                        End If

                        col += 1
                        If IsDBNull(drow("PARCRESTANTES")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PARCRESTANTES")
                        End If

                        col += 1
                        If IsDBNull(drow("OPVLROPCZ")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("OPVLROPCZ"), 2)
                        End If

                        col += 1
                        If IsDBNull(drow("PAVLRPARCELA")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("PAVLRPARCELA"), 2)
                        End If
                        ...
                        If IsDBNull(drow("PANRPARC")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PANRPARC")
                        End If
                        ...
                        If IsDBNull(drow("PADTVCTO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PADTVCTO")
                        End If

                        col += 1
                        If IsDBNull(drow("SALDOQUITACAO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("SALDOQUITACAO"), 2)
                        End If
                        ...
                        col += 1
                        If IsDBNull(drow("GARANTIA")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("GARANTIA")
                        End If

                        col += 1
                        If IsDBNull(drow("DESCRGARANTIA")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("DESCRGARANTIA")
                        End If

                        col += 1
                        If IsDBNull(drow("VLRGARANTIA")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLRGARANTIA"), 2)
                        End If
                        ...
                        If IsDBNull(drow("OBSERVACAO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("OBSERVACAO")
                        End If

                        col += 1
                        If IsDBNull(drow("TPPAGTO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("TPPAGTO")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_CP_COLAB_AGENTES_SINTETICO]", "SCR_CP_COLAB_AGENTES_SINTETICO", strConn)
        ...
        Protected Sub btnCarregar_Click(sender As Object, e As EventArgs)
            Try
                ...
                'ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Em construção!' ,'Relatório em desenvolvimento.');", True)
                BindGridView1Data()
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("CP_Colab_Agentes_Sintetico_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("<p>CP - Colaboradores e Agentes - Sintético</p> ")
