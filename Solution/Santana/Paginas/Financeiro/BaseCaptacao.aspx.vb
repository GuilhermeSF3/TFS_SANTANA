    ...
    Public Class BaseCaptacao
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ...
                txtDataAplicacao.Text = UltimoDiaUtilMes(ultimoDiaMesAnterior)
        ...
        Private Function UltimoDiaUtilMes(Data As Date) As String
        ...
        Protected Sub btnDataAnteriorApp_Click(sender As Object, e As EventArgs)

            Dim diaAnterior As Date = Convert.ToDateTime(txtDataAplicacao.Text)
            diaAnterior = diaAnterior.AddMonths(-1)
        ...
        Protected Sub btnProximaDataApp_Click(sender As Object, e As EventArgs)
        ...
        Private Sub GridView1_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridView1.RowDataBound
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.Header Then
                    ...
                    Dim dataMesAnterior = UltimoDiaUtilMes(oData.Data.AddMonths(-1))
                    e.Row.Cells(7).Text = "Valor em " & dataMesAnterior


                    Dim dataMesAtual = UltimoDiaUtilMes(oData.Data)
                    e.Row.Cells(8).Text = "Valor em " & dataMesAtual


                    Dim primeiroDiaMes As Date = Convert.ToDateTime("01/" + oData.Data.ToString("MM/yyyy"))
                    Dim ultimoDiaMesAnterior = primeiroDiaMes.AddMonths(1)
                    ultimoDiaMesAnterior = ultimoDiaMesAnterior.AddDays(-1)
                    e.Row.Cells(9).Text = "Valor Rend. " & primeiroDiaMes.ToString("dd/MM") & "-" & ultimoDiaMesAnterior.Date.ToString("dd/MM")
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        col += 1
                        If IsDBNull(drow("DIAS")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("DIAS")
                        End If

                        col += 1
                        If IsDBNull(drow("PRE_CDI")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PRE_CDI")
                        End If
                        ...
                        If IsDBNull(drow("TAXA")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("TAXA")
                        End If

                        col += 1
                        If IsDBNull(drow("GERENTE")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("GERENTE")
                        End If

                        col += 1
                        If IsDBNull(drow("VLR_ORIGINAL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_ORIGINAL"), 2)
                        End If

                        col += 1
                        If IsDBNull(drow("VLR_MES_ANT")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_MES_ANT"), 2)
                        End If

                        col += 1
                        If IsDBNull(drow("VLR_MES_ATUAL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_MES_ATUAL"), 2)
                        End If

                        col += 1
                        If IsDBNull(drow("VLR_REND_MES")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_REND_MES"), 2)
                        End If

                        col += 1
                        If IsDBNull(drow("VLR_PROJETADO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_PROJETADO"), 2)
                        End If
                        ...
                        col += 1
                        If IsDBNull(drow("ORIGEM")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("ORIGEM")
                        End If

                        col += 1
                        If IsDBNull(drow("COD_PAPEL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("COD_PAPEL")
                        End If

                        col += 1
                        If IsDBNull(drow("CALC_TOTAL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("CALC_TOTAL")
                        End If

                        col += 1
                        If IsDBNull(drow("SERIE_OPERACAO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("SERIE_OPERACAO")
                        End If

                        col += 1
                        If IsDBNull(drow("PAPEL_CODIGO")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PAPEL_CODIGO")
                        End If

                        col += 1
                        If IsDBNull(drow("PRC_CDI_REAL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PRC_CDI_REAL")
                        End If

                        col += 1
                        If IsDBNull(drow("PRC_CDI_INTERM")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PRC_CDI_INTERM")
                        End If

                        col += 1
                        If IsDBNull(drow("PRC_CDI_FINAL")) Then
                            e.Row.Cells(col).Text = ""
                        Else
                            e.Row.Cells(col).Text = drow("PRC_CDI_FINAL")
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim oDataApp As New CDataHora(Convert.ToDateTime(txtDataAplicacao.Text))
            ...
            table = ClassBD.GetExibirGrid("[SCR_financeiro_base] '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "', '" & oDataApp.Data.ToString("yyyyMMdd") & "', " & valorCdi, "FinCaptacao", strConn)
        ...
        Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
            ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Base de Captação!' ,'Todas as Captações que trafegam no IOPEN no período.');", True)
        End Sub
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("BaseCaptacao_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Base de Captação - Financeiro")
