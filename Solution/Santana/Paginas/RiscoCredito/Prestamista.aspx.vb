...
Namespace Paginas.RiscoCredito
    ...
    Public Class Prestamista
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("DATA_REFERENCIA")) Then
                            e.Row.Cells(0).Text = ""
                        Else
                            e.Row.Cells(0).Text = drow("DATA_REFERENCIA")
                        End If
                        ...
                        If IsDBNull(drow("OCUPACAO")) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = drow("OCUPACAO")
                        End If

                        If IsDBNull(drow("FONE_RESIDENCIAL")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("FONE_RESIDENCIAL")
                        End If

                        If IsDBNull(drow("FONE_CELULAR")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("FONE_CELULAR")
                        End If

                        If IsDBNull(drow("OPERACAO")) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = drow("OPERACAO")
                        End If

                        If IsDBNull(drow("COBRADORA")) Then
                            e.Row.Cells(8).Text = ""
                        Else
                            e.Row.Cells(8).Text = drow("COBRADORA")
                        End If

                        If IsDBNull(drow("VALOR_PRESTAMISTA")) Then
                            e.Row.Cells(9).Text = ""
                        Else
                            e.Row.Cells(9).Text = drow("VALOR_PRESTAMISTA")
                        End If

                        If IsDBNull(drow("QTDE_PARCELAS_OPERACAO")) Then
                            e.Row.Cells(10).Text = ""
                        Else
                            e.Row.Cells(10).Text = drow("QTDE_PARCELAS_OPERACAO")
                        End If

                        If IsDBNull(drow("QTDE_PARCELAS_ABERTO")) Then
                            e.Row.Cells(11).Text = ""
                        Else
                            e.Row.Cells(11).Text = drow("QTDE_PARCELAS_ABERTO")
                        End If

                        If IsDBNull(drow("DATA_BASE_OPERACAO")) Then
                            e.Row.Cells(12).Text = ""
                        Else
                            e.Row.Cells(12).Text = drow("DATA_BASE_OPERACAO")
                        End If

                        If IsDBNull(drow("ULTIMO_VCTO_OPERACAO")) Then
                            e.Row.Cells(13).Text = ""
                        Else
                            e.Row.Cells(13).Text = drow("ULTIMO_VCTO_OPERACAO")
                        End If

                        If IsDBNull(drow("DIAS_ATRASO")) Then
                            e.Row.Cells(14).Text = ""
                        Else
                            e.Row.Cells(14).Text = drow("DIAS_ATRASO")
                        End If

                        If IsDBNull(drow("SALDO_DEVEDOR_NA_REF")) Then
                            e.Row.Cells(15).Text = ""
                        Else
                            e.Row.Cells(15).Text = drow("SALDO_DEVEDOR_NA_REF")
                        End If

                        If IsDBNull(drow("DATA_ULTIMO_PGTO")) Then
                            e.Row.Cells(16).Text = ""
                        Else
                            e.Row.Cells(16).Text = drow("DATA_ULTIMO_PGTO")
                        End If

                        If IsDBNull(drow("DIAS_ULTIMO_PGTO")) Then
                            e.Row.Cells(17).Text = ""
                        Else
                            e.Row.Cells(17).Text = drow("DIAS_ULTIMO_PGTO")
                        End If
                    End If
                End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_PRESTAMISTA] '" & ddlAgente.SelectedValue.PadLeft(6, "0") & "'", "PRESTAMISTA", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Prestamista_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Prestamista -  Agente " & ddlAgente.SelectedItem.Text & " </p> ")
