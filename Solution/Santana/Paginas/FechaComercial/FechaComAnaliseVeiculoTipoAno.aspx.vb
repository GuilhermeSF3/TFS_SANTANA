...
Public Class FechaComAnaliseVeiculoTipoAno
    ...
    Private Sub Carrega_Relatorio()
        ...
        Try
            ...
            ddlRelatorio.Items.Insert(2, New ListItem("% Quantidade", "3"))
            ddlRelatorio.Items.Insert(3, New ListItem("% Valor", "4"))
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    If IsDBNull(drow("fx1") Or drow("fx1") = 0) Then
                        e.Row.Cells(1).Text = ""
                    Else
                        e.Row.Cells(1).Text = CNumero.FormataNumero(drow("fx1"), decimais)
                    End If

                    If IsDBNull(drow("fx2") Or drow("fx2") = 0) Then
                        e.Row.Cells(2).Text = ""
                    Else
                        e.Row.Cells(2).Text = CNumero.FormataNumero(drow("fx2"), decimais)
                    End If

                    If IsDBNull(drow("fx3") Or drow("fx3") = 0) Then
                        e.Row.Cells(3).Text = ""
                    Else
                        e.Row.Cells(3).Text = CNumero.FormataNumero(drow("fx3"), decimais)
                    End If

                    If IsDBNull(drow("fx4") Or drow("fx4") = 0) Then
                        e.Row.Cells(4).Text = ""
                    Else
                        e.Row.Cells(4).Text = CNumero.FormataNumero(drow("fx4"), decimais)
                    End If

                    If IsDBNull(drow("fx5") Or drow("fx5") = 0) Then
                        e.Row.Cells(5).Text = ""
                    Else
                        e.Row.Cells(5).Text = CNumero.FormataNumero(drow("fx5"), decimais)
                    End If
    ...
    Private Function GetData() As DataTable
        ...
        Dim table As DataTable = Util.ClassBD.GetExibirGrid("[scr_COMERCIAL_PLANO_ANO] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', 'V', '" & ddlRelatorio.SelectedValue & "','" & ddlVeiculo.SelectedValue & "'", "FechaComAnaliseVeiculoTipoAno", strConn)
    ...
    Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Not IsNothing(GridView1.HeaderRow) Then
                ...
                Dim filename As String = String.Format("FechaComAnaliseVeiculoTipoAno_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
