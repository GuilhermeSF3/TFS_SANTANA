...
Public Class ComissaoSintetico
    ...
    Private Sub Carrega_Opcoes()
        Try
            ddlFiltros.Items.Insert(0, New ListItem("Todos", "0"))
            ddlFiltros.Items.Insert(1, New ListItem("Agente", "1"))
            ddlFiltros.Items.Insert(2, New ListItem("Dínamo", "2"))
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    col += 1
                    If IsDBNull(drow("QTD_OPERACOES")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = drow("QTD_OPERACOES")
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("REPASSE")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("REPASSE"), 2)
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("ADIANTAMENTO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("ADIANTAMENTO"), 2)
                    End If

                    col += 1
                    If IsDBNull(drow("SALDO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("SALDO"), 2)
                    End If
    ...
    Private Function GetData() As DataTable
        ...
        If ddlFiltros.SelectedValue = 0 Or ddlFiltros.SelectedValue = 2 Then

            GeraComissao(Convert.ToDateTime(txtDataDe.Text), Convert.ToDateTime(txtDataAte.Text))
            GeraComissaoDinamo(Convert.ToDateTime(txtDataDe.Text), Convert.ToDateTime(txtDataAte.Text))

        End If

        table = ClassBD.GetExibirGrid("[SCR_COMISSAO_SINTETICO] '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "'," & ddlFiltros.SelectedValue, "Comissao", strConn)

        Return table

    End Function

    Private Sub GeraComissao(dtInicio As Date, dtFim As Date)
    ...
    Private Sub ExportExcel(objGrid As GridView)
        ...
        Try
            ...
            If Not IsNothing(objGrid.HeaderRow) Then
                ...
                Dim filename As String = String.Format("Comissão_Sintético_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                ...
                Using sw As New StringWriter()
                    ...
                    Response.Write("Comissão Sintético - DE: " & txtDataDe.Data & " ATÉ: " & txtDataAte.Data)
