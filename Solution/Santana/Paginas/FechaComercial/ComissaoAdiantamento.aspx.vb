...
Public Class ComissaoAdiantamento
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            txtDataDe.Text = (DateTime.Now).ToString("01/MM/yyyy")
            txtDataAte.Text = (DateTime.Now).ToString("15/MM/yyyy")
    ...
    Protected Sub btnDataAnteriorDe_Click(sender As Object, e As EventArgs)
        ...
        txtDataDe.Text = diaAnterior.ToString("01/MM/yyyy")

        Dim diaAnteriorAte As Date = Convert.ToDateTime(txtDataAte.Text)
        diaAnteriorAte = diaAnteriorAte.AddMonths(-1)
        txtDataAte.Text = diaAnteriorAte.ToString("15/MM/yyyy")
    ...
    Protected Sub btnProximaDataDe_Click(sender As Object, e As EventArgs)
        ...
        txtDataDe.Text = proximoDia.ToString("01/MM/yyyy")

        Dim proximoDiaAte As Date = Convert.ToDateTime(txtDataAte.Text)
        proximoDiaAte = proximoDiaAte.AddMonths(+1)
        txtDataAte.Text = proximoDiaAte.ToString("15/MM/yyyy")

    End Sub

    Protected Sub btnDataAnteriorAte_Click(sender As Object, e As EventArgs)

        Dim diaAnteriorDe As Date = Convert.ToDateTime(txtDataDe.Text)
        diaAnteriorDe = diaAnteriorDe.AddMonths(-1)
        txtDataDe.Text = diaAnteriorDe.ToString("01/MM/yyyy")

        Dim diaAnterior As Date = Convert.ToDateTime(txtDataAte.Text)
        diaAnterior = diaAnterior.AddMonths(-1)
        txtDataAte.Text = diaAnterior.ToString("15/MM/yyyy")

    End Sub

    Protected Sub btnProximaDataAte_Click(sender As Object, e As EventArgs)

        Dim proximoDiaDe As Date = Convert.ToDateTime(txtDataDe.Text)
        proximoDiaDe = proximoDiaDe.AddMonths(+1)
        txtDataDe.Text = proximoDiaDe.ToString("01/MM/yyyy")

        Dim proximoDia As Date = Convert.ToDateTime(txtDataAte.Text)
        proximoDia = proximoDia.AddMonths(+1)
        txtDataAte.Text = proximoDia.ToString("15/MM/yyyy")
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    If IsDBNull(drow("OPERACAO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = drow("OPERACAO")
                    End If

                    col += 1
                    If IsDBNull(drow("ICM")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("ICM"), 2)
                    End If

                    col += 1
                    If IsDBNull(drow("PRC_REPASSE")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("PRC_REPASSE"), 2)
                    End If
                    ...
                    If IsDBNull(drow("VLR_LIQUIDO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_LIQUIDO"), 2)
                    End If

                    col += 1
                    If IsDBNull(drow("VLR_OPERADO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("VLR_OPERADO"), 2)
                    End If
                    ...
                    If IsDBNull(drow("COMISSAO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("COMISSAO"), 2)
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("COMISSAO_SUB")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("COMISSAO_SUB"), 2)
                    End If
    ...
    Private Function GetData() As DataTable
        ...
        table = ClassBD.GetExibirGrid("[SCR_COMISSAO_ADIANTAMENTO] '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "'", "Comissao", strConn)
    ...
    Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Comissão' ,'Cálculo Comissão.');", True)
    End Sub
    ...
    Private Sub ExportExcel(objGrid As GridView)
        ...
        Try
            ...
            If Not IsNothing(objGrid.HeaderRow) Then
                ...
                Dim filename As String = String.Format("Comissão_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                ...
                Using sw As New StringWriter()
                    ...
                    Response.Write("Comissão - DE: " & txtDataDe.Data & " ATÉ: " & txtDataAte.Data)
