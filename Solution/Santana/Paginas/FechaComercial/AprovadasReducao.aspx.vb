...
Public Class AprovadasReducao
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    Else
                        e.Row.Cells(col).Text = drow("EPDTLNC")
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("PPVLRSOL")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("PPVLRSOL"), 0)
                    End If

                    col += 1
                    If IsDBNull(drow("PPVLRLIB")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("PPVLRLIB"), 0)
                    End If
                    ...
                    If IsDBNull(drow("LOJA")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = drow("LOJA")
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("CGVLRGAR")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = CNumero.FormataNumero(drow("CGVLRGAR"), 0)
                    End If
                    ...
                    col += 1
                    If IsDBNull(drow("EPUSUARIO")) Then
                        e.Row.Cells(col).Text = ""
                    Else
                        e.Row.Cells(col).Text = drow("EPUSUARIO")
                    End If
    ...
    Private Function GetData() As DataTable
        ...
        table = ClassBD.GetExibirGrid("[SCR_APROVADAS_REDUCAO] '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "'", "AprovadasReducao", strConn)
    ...
    Protected Sub btnHelp_Click(sender As Object, e As EventArgs)
        ScriptManager.RegisterStartupScript(Me.Page, Me.GetType(), "tmp", "Alerta('Propostas - Aprovadas com Reduação' ,'Todas as Propostas que foram aprovadas com redução.');", True)
    End Sub
    ...
    Protected Sub btnCarregar_Click(sender As Object, e As EventArgs)
        Try
            ...
            BindGridView1Data()
            txtQtdeReducao.Text = CType(GridView1.DataSource, System.Data.DataTable).Rows.Count
            ...
            Dim comando As SqlCommand = New SqlCommand(
                "SCR_APROVADAS_REDUCAO_TTL '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "'", conexao)

            comando.Connection.Open()
            Dim ddlValues As SqlDataReader
            ddlValues = comando.ExecuteReader()
            ddlValues.Read()
            txtQtdeTotal.Text = ddlValues("TOTAL").ToString()
            ddlValues.Close()
            ...
            Dim conexao1 As New SqlConnection(strConn1)

            Dim comando1 As SqlCommand = New SqlCommand(
                "SCR_APROVADAS_REDUCAO_TTL_APR '" & oDataDe.Data.ToString("yyyyMMdd") & "', '" & oDataAte.Data.ToString("yyyyMMdd") & "'", conexao1)

            comando1.Connection.Open()
            Dim ddlValues1 As SqlDataReader
            ddlValues1 = comando1.ExecuteReader()
            ddlValues1.Read()
            txtQtdeAprovadas.Text = ddlValues1("APROVADA").ToString()
            ddlValues1.Close()
    ...
    Private Sub ExportExcel(objGrid As GridView)
        ...
        Try
            ...
            If Not IsNothing(objGrid.HeaderRow) Then
                ...
                Dim filename As String = String.Format("AprovadasReducao_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                ...
                Using sw As New StringWriter()
                    ...
                    Response.Write("Aprovadas com redução - DE: " & txtDataDe.Data & " ATÉ: " & txtDataAte.Data)
