...
Public Class FechaComAnaliseVeiculoPlano
    ...
    Private Sub Carrega_Veiculo()

        Try

            ddlVeiculo.Items.Insert(0, New ListItem("Leves", "1"))
            ddlVeiculo.Items.Insert(1, New ListItem("Total", "2"))
    ...
    Private Sub Carrega_Relatorio()
        ...
        Try
            ...
            ddlRelatorio.Items.Insert(0, New ListItem("Plano Qtde", "1"))
            ddlRelatorio.Items.Insert(1, New ListItem("Plano Valor", "2"))
    ...
    Private Sub GridView1_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GridView1.RowDataBound
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.Header Then
                ...
                Dim xx As DataTable = DataGridView()

                For Each drow As DataRow In xx.Rows

                    ' If Not IsDBNull(drow("ORDEM") And drow("ORDEM") = 1) Then   'TODO: verificar como filtrar os dados do cabeçalho
                    ...
                    If IsDBNull(drow("fx1")) Then
                        e.Row.Cells(1).Text = ""
                    Else
                        e.Row.Cells(1).Text = drow("fx1")
                    End If

                    If IsDBNull(drow("fx2")) Then
                        e.Row.Cells(2).Text = ""
                    Else
                        e.Row.Cells(2).Text = drow("fx2")
                    End If

                    If IsDBNull(drow("fx3")) Then
                        e.Row.Cells(3).Text = ""
                    Else
                        e.Row.Cells(3).Text = drow("fx3")
                    End If

                    If IsDBNull(drow("fx4")) Then
                        e.Row.Cells(4).Text = ""
                    Else
                        e.Row.Cells(4).Text = drow("fx4")
                    End If

                    If IsDBNull(drow("fx5")) Then
                        e.Row.Cells(5).Text = ""
                    Else
                        e.Row.Cells(5).Text = drow("fx5")
                    End If
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    ElseIf drow("COR_LINHA") = "A1" Then
                        cor1 = Drawing.ColorTranslator.FromOle(&HFFF6D2)
                    ElseIf drow("COR_LINHA") = "A2" Then
                        cor1 = Drawing.ColorTranslator.FromOle(&HE3C5C5)
                    Else
                        cor1 = e.Row.Cells(0).BackColor
                    End If
                    ...
                    If IsDBNull(drow("fx1") Or drow("fx1") = 0) Then
                        e.Row.Cells(1).Text = ""
                    Else
                        e.Row.Cells(1).Text = CNumero.FormataNumero(drow("fx1"), 2)
                    End If
                    e.Row.Cells(1).BackColor = cor1

                    If IsDBNull(drow("fx2") Or drow("fx2") = 0) Then
                        e.Row.Cells(2).Text = ""
                    Else
                        e.Row.Cells(2).Text = CNumero.FormataNumero(drow("fx2"), 2)
                    End If
                    e.Row.Cells(2).BackColor = cor1

                    If IsDBNull(drow("fx3") Or drow("fx3") = 0) Then
                        e.Row.Cells(3).Text = ""
                    Else
                        e.Row.Cells(3).Text = CNumero.FormataNumero(drow("fx3"), 2)
                    End If
                    e.Row.Cells(3).BackColor = cor1

                    If IsDBNull(drow("fx4") Or drow("fx4") = 0) Then
                        e.Row.Cells(4).Text = ""
                    Else
                        e.Row.Cells(4).Text = CNumero.FormataNumero(drow("fx4"), 2)
                    End If
                    e.Row.Cells(4).BackColor = cor1

                    If IsDBNull(drow("fx5") Or drow("fx5") = 0) Then
                        e.Row.Cells(5).Text = ""
                    Else
                        e.Row.Cells(5).Text = CNumero.FormataNumero(drow("fx5"), 2)
                    End If
    ...
    Private Function GetData() As DataTable
        ...
        Dim table As DataTable = Util.ClassBD.GetExibirGrid("[scr_COMERCIAL_PLANO] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', 'V', '" & ddlRelatorio.SelectedValue & "','" & ddlVeiculo.SelectedValue & "'", "FechaComAnaliseVeiculoPlano", strConn)
    ...
    Protected Sub BindGraphData()
        ...
        Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
        Dim objData As DataTable = ClassBD.GetExibirGrid("[scr_COMERCIAL_PLANO_GRAF1] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', 'V', '" & ddlRelatorio.SelectedValue & "','" & ddlVeiculo.SelectedValue & "'", "FechaComAnaliseVeiculoPlano", strConn)


        For Each oDataRow As DataRow In objData.Rows


            _hfDataSerie1 = _hfDataSerie1 & "<vc:DataSeries LegendText= """ & oDataRow("DESCR") & """ RenderAs=""Column"" MarkerType=""Circle"" SelectionEnabled=""True"" LineThickness=""3"">"
            _hfDataSerie1 = _hfDataSerie1 & "<vc:DataSeries.DataPoints> "

            If Not IsDBNull(oDataRow("fx1")) Then
                _hfDataSerie1 = _hfDataSerie1 & "<vc:DataPoint  LabelEnabled=""True"" AxisXLabel= ""0-18""  YValue=""" & Replace((oDataRow("fx1") / 1.0), ",", ".") & """/> "
            End If
            If Not IsDBNull(oDataRow("fx2")) Then
                _hfDataSerie1 = _hfDataSerie1 & "<vc:DataPoint  LabelEnabled=""True"" AxisXLabel= ""19-24""  YValue=""" & Replace((oDataRow("fx2") / 1.0), ",", ".") & """/> "
            End If
            If Not IsDBNull(oDataRow("fx3")) Then
                _hfDataSerie1 = _hfDataSerie1 & "<vc:DataPoint  LabelEnabled=""True"" AxisXLabel= ""25-37""  YValue=""" & Replace((oDataRow("fx3") / 1.0), ",", ".") & """/> "
            End If
            If Not IsDBNull(oDataRow("fx4")) Then
                _hfDataSerie1 = _hfDataSerie1 & "<vc:DataPoint  LabelEnabled=""True"" AxisXLabel= ""37-48""  YValue=""" & Replace((oDataRow("fx4") / 1.0), ",", ".") & """/> "
            End If
    ...
    Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Not IsNothing(GridView1.HeaderRow) Then
                ...
                Dim filename As String = String.Format("FechaComAnaliseVeiculoPlano_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
    ...
    Public Function GetXaml1() As [String]
        ...
        myXaml = "<vc:Chart  xmlns:vc=""clr-namespace:Visifire.Charts;assembly=SLVisifire.Charts"" AnimatedUpdate=""True"" Width=""1200"" Height=""400"" Theme=""Theme1"" BorderBrush=""Gray"" IndicatorEnabled=""True""   ToolBarEnabled=""True""  >"
        myXaml = myXaml & "<vc:Chart.Titles>"

        myXaml = myXaml & "<vc:Title Text=""" & "Análise de Veículos Plano - " & ddlVeiculo.SelectedItem.ToString() & " - " & ddlRelatorio.SelectedItem.ToString() & """/> "
