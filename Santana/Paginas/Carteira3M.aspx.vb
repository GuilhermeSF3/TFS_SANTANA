...
Public Class Carteira3M
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    e.Row.Cells(0).Text = drow("CLASSE")
                    e.Row.Cells(1).Text = If(IsDBNull(drow("QTDM3")), 0, CNumero.FormataNumero(drow("QTDM3"), 0))
                    e.Row.Cells(2).Text = If(IsDBNull(drow("VLRM3")), 0, CNumero.FormataNumero(drow("VLRM3"), 0))
                    e.Row.Cells(3).Text = If(IsDBNull(drow("TM3")), 0, CNumero.FormataNumero(drow("TM3"), 2))
                    e.Row.Cells(4).Text = If(IsDBNull(drow("QTDM2")), 0, CNumero.FormataNumero(drow("QTDM2"), 0))
                    e.Row.Cells(5).Text = If(IsDBNull(drow("VLRM2")), 0, CNumero.FormataNumero(drow("VLRM2"), 0))
                    e.Row.Cells(6).Text = If(IsDBNull(drow("TM2")), 0, CNumero.FormataNumero(drow("TM2"), 2))
                    e.Row.Cells(7).Text = If(IsDBNull(drow("QTDM1")), 0, CNumero.FormataNumero(drow("QTDM1"), 0))
                    e.Row.Cells(8).Text = If(IsDBNull(drow("VLRM1")), 0, CNumero.FormataNumero(drow("VLRM1"), 0))
                    e.Row.Cells(9).Text = If(IsDBNull(drow("TM1")), 0, CNumero.FormataNumero(drow("TM1"), 2))
    ...
    Private Function GetData() As DataTable
        ...
        Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
        Dim Table As DataTable = Util.ClassBD.GetExibirGrid("[scr_CARTEIRA_3M] '" & Convert.ToDateTime(txtData.Text).ToString("yyyyMMdd") & "', '" & ddlAgente.SelectedValue & "','" & ddlCobradora.SelectedValue & "'", "IndicePerformance", strConn)

        Quantidade3 = Table.Compute("sum(QTDM3)", "")
        ValorRisco3 = Table.Compute("sum(VLRM3)", "")

        Quantidade2 = Table.Compute("sum(QTDM2)", "")
        ValorRisco2 = Table.Compute("sum(VLRM2)", "")

        Quantidade1 = Table.Compute("sum(QTDM1)", "")
        ValorRisco1 = Table.Compute("sum(VLRM1)", "")
    ...
    Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
        ...
        Try
            ...
            If Not IsNothing(GridView1.HeaderRow) Then
                ...
                Dim filename As String = String.Format("Carteira3M_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
