...
Public Class RrMensalCUBO
        ...
        Try
            ...
            If e.Row.RowType = DataControlRowType.DataRow Then
                If Not e.Row.DataItem Is Nothing Then
                    ...
                    'e.Row.Cells(2).Text = CNumero.FormataNumero(Convert.ToInt32(drow("AA_PERC")), 2)
                    'e.Row.Cells(2).ForeColor = Cor

                    'e.Row.Cells(4).Text = CNumero.FormataNumero(Convert.ToInt32(drow("A_PERC")), 2)
    ...
    Protected Sub GridView1_RowCreated(sender As Object, e As GridViewRowEventArgs)
        Try
            ...
            '    tclCelula.Text = "Classe de Atrasso"
            ...
            '    tclCelula = New TableCell()
            '    tclCelula.Text = "D - 61 a 120 d"
            '    tclCelula.ColumnSpan = 3
            '    gvrObjetoLinha.Cells.Add(tclCelula)
            '    grvObjeto.Controls(0).Controls.AddAt(0, gvrObjetoLinha)

            '    tclCelula = New TableCell()
            '    tclCelula.Text = "E - 121 a 150 d"
            '    tclCelula.ColumnSpan = 3
            '    gvrObjetoLinha.Cells.Add(tclCelula)
            '    grvObjeto.Controls(0).Controls.AddAt(0, gvrObjetoLinha)

            '    tclCelula = New TableCell()
            '    tclCelula.Text = "F - 151 a 180 d"
            '    tclCelula.ColumnSpan = 3
            '    gvrObjetoLinha.Cells.Add(tclCelula)
            '    grvObjeto.Controls(0).Controls.AddAt(0, gvrObjetoLinha)

            '    tclCelula = New TableCell()
            '    tclCelula.Text = "G - 181 a 210 d"
            '    tclCelula.ColumnSpan = 3
            '    gvrObjetoLinha.Cells.Add(tclCelula)
            '    grvObjeto.Controls(0).Controls.AddAt(0, gvrObjetoLinha)

            '    tclCelula = New TableCell()
            '    tclCelula.Text = "H - 181 a 210 d"
            '    tclCelula.ColumnSpan = 3
            '    gvrObjetoLinha.Cells.Add(tclCelula)
            '    grvObjeto.Controls(0).Controls.AddAt(0, gvrObjetoLinha)
    ...
    Protected Sub BindGridView1Data(ByVal strSortField As String)
        ...
        ' Convert.ToDateTime(txtData.Text).ToString("yyyyMMdd") 
        ...
        ' com localizar
        ' GridView1.DataSource = Util.ClassBD.GetExibirGrid("SELECT * FROM PDD_CUBO WHERE DT_FECHA='" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "' and [NUM_CUBO] =" & ddlAgente.SelectedValue & "' and [M0_DESCR1] like '%" & TxtLocalizar.Text & "%' order by " & strSortField, "RRMENSALcubo", strConn)

        ' sem localizar
        ' GridView1.DataSource = Util.ClassBD.GetExibirGrid("SELECT * FROM PDD_CUBO WHERE DT_FECHA='" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "' and [NUM_CUBO] ='" & ddlAgente.SelectedValue & "' order by " & strSortField, "RRMENSALcubo", strConn)
        ...
        Try
            ...
            If Not IsNothing(GridView1.HeaderRow) Then
                ...
                Using sw As New StringWriter()
                    ...
                    ' GRID JAH ESTA CARREGADO, DESNECESSARIO          ' MOSTRANDO SOMENTE 1A PAGINA
                    ' CARREGAR A TABELA NOVAMENTE, SEM PAGINACAO PARA ENVIAR TUDO AO EXCEL
