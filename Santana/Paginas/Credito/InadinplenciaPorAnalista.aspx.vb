...
Namespace Paginas.Credito
    ...
    Public Class InadinplenciaPorAnalista
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        'If IsDBNull(drow("contrato")) OrElse drow("contrato") = 0 Then
                        '    e.Row.Cells(2).Text = ""
                        'Else
                        '    e.Row.Cells(2).Text = CNumero.FormataNumero(drow("contrato"), 0)
                        'End If

                        e.Row.Cells(1).Text = drow("DT_prop")
                        'If IsDBNull(drow("DT_prop")) Then
                        '    e.Row.Cells(0).Text = ""
                        'Else
                        '    e.Row.Cells(0).Text = drow("DT_prop")
                        'End If

                        e.Row.Cells(2).Text = drow("analista")
                        ...
                        e.Row.Cells(3).Text = drow("proposta")
                        'If IsDBNull(drow("proposta")) OrElse drow("proposta") = 0 Then
                        '    e.Row.Cells(3).Text = ""
                        'Else
                        '    e.Row.Cells(3).Text = drow("proposta")
                        'End If
                        ...
                        e.Row.Cells(7).Text = drow("dias_atraso")
                        'If IsDBNull(drow("dias_atraso")) OrElse drow("dias_atraso") = 0 Then
                        '    e.Row.Cells(7).Text = ""
                        'Else
                        '    e.Row.Cells(7).Text = CNumero.FormataNumero(drow("dias_atraso"), 0)
                        'End If

                        e.Row.Cells(8).Text = CNumero.FormataNumero(drow("vlr_parcela"), 2)
                        'If IsDBNull(drow("vlr_parcela")) OrElse drow("vlr_parcela") = 0 Then
                        '    e.Row.Cells(8).Text = ""
                        'Else
                        '    e.Row.Cells(8).Text = CNumero.FormataNumero(drow("vlr_parcela"), 2)
                        'End If

                        e.Row.Cells(9).Text = drow("qtd_parc_atraso")
                        'If IsDBNull(drow("qtd_parc_atraso")) OrElse drow("qtd_parc_atraso") = 0 Then
                        '    e.Row.Cells(9).Text = ""
                        'Else
                        '    e.Row.Cells(9).Text = CNumero.FormataNumero(drow("qtd_parc_atraso"), 0)
                        'End If

                        e.Row.Cells(10).Text = drow("atraso_ANTIGO")
                        'If IsDBNull(drow("atraso_ANTIGO")) OrElse drow("atraso_ANTIGO") = 0 Then
                        '    e.Row.Cells(10).Text = ""
                        'Else
                        '    e.Row.Cells(10).Text = drow("atraso_ANTIGO")
                        'End If
        ...
        Private Function GetData() As DataTable
            ...
            table = Util.ClassBD.GetExibirGrid("[SCR_CREDITO_INADIMPL] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 4, 2) & Left(txtDataDe.Text, 2) & _
                                                                "', '" & Right(txtDataAte.Text, 4) & Mid(txtDataAte.Text, 4, 2) & Left(txtDataAte.Text, 2) & _
                                                                "', '" & ddlAnalista.SelectedValue & _
                                                                "', " & txtValorDe.Text & _
                                                                ", " & txtValorAte.Text & _
                                                                "", "INADINPLANALISTA", strConn)
        ...
        Private Sub ExportExcel(objGrid As GridView)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Inadinplencia_Analista_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
