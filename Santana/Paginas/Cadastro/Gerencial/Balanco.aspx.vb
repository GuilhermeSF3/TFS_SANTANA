    ...
    Public Class Balanco
        ...
        Private Sub Carrega_Relatorio()
            ...
            Try
                ...
                Dim connection As New SqlConnection(strConn)
                Dim command As SqlCommand = New SqlCommand( _
                "SELECT COD_RPT, DESCR FROM GER_RELATORIO (NOLOCK) WHERE MODELO=3", connection)
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        If IsDBNull(drow("cod_conta_at")) Then
                            e.Row.Cells(2).Text = ""
                        Else
                            e.Row.Cells(2).Text = drow("cod_conta_at")
                        End If

                        If IsDBNull(drow("DESCR_at")) Then
                            e.Row.Cells(3).Text = ""
                        Else
                            e.Row.Cells(3).Text = drow("DESCR_at")
                        End If

                        If IsDBNull(drow("vlr_Ativo") Or drow("vlr_Ativo") = 0) Then
                            e.Row.Cells(4).Text = ""
                        Else
                            e.Row.Cells(4).Text = CNumero.FormataNumero(drow("vlr_Ativo"), 2)
                        End If

                        If IsDBNull(drow("cod_conta_pas")) Then
                            e.Row.Cells(5).Text = ""
                        Else
                            e.Row.Cells(5).Text = drow("cod_conta_pas")
                        End If

                        If IsDBNull(drow("DESCR_pas")) Then
                            e.Row.Cells(6).Text = ""
                        Else
                            e.Row.Cells(6).Text = drow("DESCR_pas")
                        End If

                        If IsDBNull(drow("vlr_passivo") Or drow("vlr_passivo") = 0) Then
                            e.Row.Cells(7).Text = ""
                        Else
                            e.Row.Cells(7).Text = CNumero.FormataNumero(drow("vlr_passivo"), 2)
                        End If
        ...
        Private Function GetData() As DataTable
            ...
            Dim strConn As String = ConfigurationManager.AppSettings("ConexaoPrincipal")
            Dim table As DataTable = Util.ClassBD.GetExibirGrid("[SCR_Gerencial_balanco] '" & Right(txtData.Text, 4) & Mid(txtData.Text, 4, 2) & Left(txtData.Text, 2) & "', '" & ddlRelatorio.SelectedValue & "','" & ddlEmpresa.SelectedValue & "'", "Balanco", strConn)
        ...
        Protected Sub btnExcel_Click(sender As Object, e As EventArgs)
            ...
            Try
                ...
                If Not IsNothing(GridView1.HeaderRow) Then
                    ...
                    Dim filename As String = String.Format("Balanco_{0}_{1}_{2}.xls", DateTime.Today.Day.ToString(), DateTime.Today.Month.ToString(), DateTime.Today.Year.ToString())
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Balanço Ref " & txtData.Text & " - Relatório " & ddlRelatorio.SelectedItem.Text)
