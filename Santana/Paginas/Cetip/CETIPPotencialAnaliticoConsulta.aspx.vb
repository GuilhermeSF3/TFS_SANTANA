...
Namespace Paginas.Cetip
    ...
    Public Class CETIPPotencialAnaliticoConsulta
        ...
        Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
            ...
            If Not IsPostBack Then
                ...
                Dim Ultimo2MesAnterior As Date = Convert.ToDateTime("01/" + Now.Date.AddMonths(-2).ToString("MM/yyyy"))
                txtDataDe.Text = Mid(Convert.ToString(Ultimo2MesAnterior), 4, 7)
                ' Mid(Convert.ToString(Ultimo2MesAnterior), 4, 7)
                ' UltimoDiaUtilMesAnterior(Ultimo2MesAnterior)

                Dim UltimoMesAnterior As Date = Convert.ToDateTime("01/" + Now.Date.AddMonths(-1).ToString("MM/yyyy"))
        ...
        Protected Sub btnDataAnterior1_Click(sender As Object, e As EventArgs)
            ...
            'Convert.ToDateTime(Mid(txtDataDe.Text, 4, 4) + Mid(txtDataDe.Text, 1, 2) + "01")
        ...
        Protected Sub btnProximaData1_Click(sender As Object, e As EventArgs)
            ...
            If UltimoDiaMesAnterior.Year = Now.Date.Year Then
                If UltimoDiaMesAnterior.Month <= Now.Date.Month Then
                    txtDataDe.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
                    'UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior)
                End If
            ElseIf UltimoDiaMesAnterior.Year < Now.Date.Year Then
                txtDataDe.Text = Mid(UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior), 4, 8)
                'UltimoDiaUtilMesAnterior(UltimoDiaMesAnterior)
        ...
        Private Sub Carrega_lOJA()
            ...
            Try
                ...
                ddlLOJA.Items.Insert(0, New ListItem("TODAS", ""))
                ddlLOJA.Items.Insert(1, New ListItem("ATIVAS", "A"))
                ddlLOJA.Items.Insert(2, New ListItem("DESATIVADAS", "D"))
            ...
            Try
                ...
                If e.Row.RowType = DataControlRowType.DataRow Then
                    ...
                    If Not e.Row.DataItem Is Nothing Then
                        ...
                        e.Row.Cells(2).Text = drow("Loja_Ativa")
                        e.Row.Cells(3).Text = drow("Loja")
                        e.Row.Cells(4).Text = drow("Endereco")

                        e.Row.Cells(5).Text = drow("Complemento")
                        'e.Row.Cells(6).Text = drow("Numero")
                        e.Row.Cells(6).Text = drow("Bairro")
                        e.Row.Cells(7).Text = drow("Cidade")
                        e.Row.Cells(8).Text = drow("UF")
                        e.Row.Cells(9).Text = drow("CEP")

                        e.Row.Cells(10).Text = drow("Agente")
                        e.Row.Cells(11).Text = drow("Categoria")
                        e.Row.Cells(12).Text = drow("Segmento")
                        e.Row.Cells(13).Text = drow("Consorcio")
                        e.Row.Cells(14).Text = drow("TipoPessoa")
                        e.Row.Cells(15).Text = drow("Quantidade")
                        e.Row.Cells(16).Text = drow("Volume")
                    End If
                End If
        ...
        Protected Sub BindGridView1Data()
            ...
            'RESOLVE O PROBLEMA DE PAGINACAO
        ...
        Private Function GetData() As DataTable
            ...
            'table = Util.ClassBD.GetExibirGrid("[SCR_CETIP_POTENCIAL_ANALITICO_CONSULTA] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 4, 2) & "', '" & Right(txtDataATE.Text, 4) & Mid(txtDataATE.Text, 4, 2) & "', '" & txtLocalizar.Text & "', '" & txtCepDe.Text & "', '" & TxtCepAte.Text & "', '" & ddlLOJA.SelectedValue & "'", "PACETIP", strConn)
            table = Util.ClassBD.GetExibirGrid("[SCR_CETIP_POTENCIAL_ANALITICO_CONSULTA] '" & Right(txtDataDe.Text, 4) & Mid(txtDataDe.Text, 1, 2) & "', '" & Right(txtDataATE.Text, 4) & Mid(txtDataATE.Text, 1, 2) & "', '" & txtLocalizar.Text & "', '" & txtCepDe.Text & "', '" & TxtCepAte.Text & "', '" & ddlLOJA.SelectedValue & "'", "PACETIP", strConn)
            ...
            Try
                ...
                If Not IsNothing(objGrid.HeaderRow) Then
                    ...
                    Using sw As New StringWriter()
                        ...
                        Response.Write("Cetip Potencial Analitico  - Datas de: " + txtDataDe.Text + " até: " + txtDataATE.Text + " Filtro: " + txtLocalizar.Text + " CEP De: " + txtCepDe.Text + " até: " + TxtCepAte.Text + " Lojas: " + ddlLOJA.SelectedValue)
