...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'JOB_REMESSA_09_CFI' AND type = 'P')
   DROP PROCEDURE [dbo].[JOB_REMESSA_09_CFI]
GO   
CREATE PROCEDURE [DBO].[JOB_REMESSA_09_CFI] AS                          
...
SET @DT_DE = CONVERT(VARCHAR(10),DATEADD(DAY,-1,GETDATE()),121)
SET @DT_ATE = CONVERT(VARCHAR(10),GETDATE(),121)

SELECT @DT_DE = CASE WHEN DATEPART(WEEKDAY,@DT_DE) = 1 THEN DATEADD(D,-2,@DT_DE) ELSE @DT_DE END

SELECT @DT_FERI = FEDTFER FROM CDCSANTANAMICROCREDITO..TFERI (NOLOCK) WHERE FEDTFER = CONVERT(VARCHAR(10),@DT_DE,121)
IF DATEPART(WEEKDAY,@DT_FERI) IN (3,4,5,6)
BEGIN
	--VERIFICAR FERIADO NOVAMENTE
	SELECT @DT_DE = DATEADD(D,-1,@DT_DE)
END
ELSE IF DATEPART(WEEKDAY,@DT_FERI) IN (2)
BEGIN
	--VERIFICAR FERIADO NOVAMENTE 
	SELECT @DT_DE = DATEADD(D,-3,@DT_DE)
END
...
'09:00:00', CONVERT(VARCHAR(10), GETDATE(), 121), '', 'CFI'
FROM 
...
WHERE P.EPDTHRFIM BETWEEN @DT_DE + '15:39:00' AND @DT_ATE + '09:00:00'
...
UPDATE  AUX_REMESSA                    
SET  ABRENAVAM= B.ABPLACA, ABCHASSI=B.ABCHASSI, ABUFLCPL=B.ABUFLCPL                  
...
WHERE DT_EMISSAO = @DT_ATE AND TIPO = 'CFI') T
