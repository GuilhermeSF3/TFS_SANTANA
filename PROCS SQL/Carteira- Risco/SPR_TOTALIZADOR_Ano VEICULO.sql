...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SPR_TOTALIZADOR_ano' AND type = 'P')
   DROP PROCEDURE [dbo].SPR_TOTALIZADOR_ano
GO


CREATE Procedure [dbo].SPR_TOTALIZADOR_ano (@DT_INI as datetime, @DT_FIM as datetime, @DT_REF as datetime) AS  
BEGIN  
  
TRUNCATE TABLE TOTAL_SPREAD  
  
/*  
DROP   TABLE TOTAL_SPREAD  
...
DELETE FROM CARTEIRA_PERDA  
  WHERE CONTRATO IN ('40102706A','353002816A','10710571A','10318990A','73000164A','90513755A')  
   ...
   UPDATE  CARTEIRA_PERDA set ANO_MOD=''  
   ...
   UPDATE  CARTEIRA_PERDA set ANO_MOD='1900' WHERE ANO_MOD in ('','S/INF') 
   UPDATE   CARTEIRA_PERDA set atividade='1900'  
    WHERE  ATIVIDADE =  ''      
 
   UPDATE CARTEIRA_PERDA  
   SET  ATIVIDADE = FX_ANO.DESCR 
   ...
   UPDATE   CARTEIRA_PERDA_RENE set ANO_MOD='1900'  
    WHERE  isnull(ANO_MOD,'') in (  '', 'S/INF')      
 
   UPDATE CARTEIRA_PERDA_RENE  
   SET  ATIVIDADE = FX_ANO.DESCR -- FAIXA  
...
SELECT DISTINCT  
  ...
  100.0        AS PER_OPERADO,  
  
    (SUM(VLR_RECEBIDO)/1000.0)  RECEBIDO,    
  100.0         AS PER_RECEBIDO,  
  
       (SUM(VLR_PREJUIZO)/1000.0)  PREJUIZO,    
  100.0         AS  PERC_PREJUIZO,  
  
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    (sum(isnull(VLR_PREJUIZO,0.0))/sum(FV_CONTRATO)*100.0) END PERC_PREJUIZO_PERDA,  
       (SUM(VLR_DESCONTO)/1000.0)  DESCONTO,   
  100.0         AS PERC_DESCONTO,  
  
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    (sum(isnull(VLR_DESCONTO,0.0))/sum(FV_CONTRATO)*100.0) END PERC_DESCONTO_PERDA,  
  (SUM(VLR_TOTAL_PERDA)/1000.0) PERDA_TOTAL,  
  100.0         AS PERC_TOTAL_PERDA,  
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    (sum(isnull(VLR_TOTAL_PERDA,0.0))/sum(FV_CONTRATO)*100.0) END    PERC_TOTAL_PERDA1,   
    (SUM(VLR_LIQUIDADO)/1000.0) LIQUIDADO,   
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    ...
    (sum(VLR_TOTAL_PERDA) /(sum(VLR_TOTAL_PERDA)+ sum(VLR_RECEBIDO))*100.0)     PERC_LIQUIDADO_PERDA,  
       (SUM(VLR_RECEBER)/1000.0)  RECEBER,      
  CASE WHEN sum(isnull(VLR_RECEBER,0.0)) = 0.0 THEN 0.0 ELSE   
  sum(VLR_RECEBER)/ sum(isnull(VLR_RECEBER,0.0))*100.0  END  PERC_ARECEBER,        
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
  (sum(VLR_RECEBER)/sum(FV_CONTRATO)*100.0)   END  PERC_ARECEBER_PERDA,  
     (SUM(VLR_PDD)/1000.0)   PDD,  
  CASE WHEN SUM(VLR_RECEBER) = 0.0 THEN 0.0 ELSE  
