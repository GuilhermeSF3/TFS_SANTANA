...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SPR_TOTALIZADOR_tipo_ano' AND type = 'P')
   DROP PROCEDURE [dbo].SPR_TOTALIZADOR_tipo_ano
GO


CREATE Procedure [dbo].SPR_TOTALIZADOR_tipo_ano (@DT_INI as datetime, @DT_FIM as datetime, @DT_REF as datetime) AS  
BEGIN  
...
alter table CARTEIRA_PERDA  add TIPO varchar(50)  
alter table CARTEIRA_PERDA_RENE add TIPO varchar(50)  
*/  
...
DELETE FROM CARTEIRA_PERDA  
 ...
 SET  TIPO = CASE WHEN DESCR_TIPO= 'Automatico'  THEN 'Leves'   
      ...
      WHEN DESCR_TIPO IN ('Utilitarios','Utilitario')  THEN 'Utilitario'   
 ...
 WHERE ISNULL(TIPO,'') = ''  
   ...
   -- ajusta ANO VEICULO  -- ANO MODELO ******************** ATIVIDADE  
   UPDATE  CARTEIRA_PERDA set ANO_MOD='1900' WHERE isnull(ANO_MOD,'') =''  
  
   UPDATE  CARTEIRA_PERDA  
   SET  ANO_MOD =  ISNULL(ABANOMOD,'S/INF')  -- com Ano Modelo  
   FROM CDCSANTANAMicroCredito..CBFIN B (NOLOCK)  
   WHERE Contrato= B.ABNROPER  
    AND B.ABNRGAR = (SELECT  MAX(B1.ABNRGAR)   
         FROM CDCSANTANAMicroCredito..CBFIN B1 (NOLOCK)   
         WHERE B1.ABNROPER = B.ABNROPER )  
    AND B.ABCNTRL = (SELECT  MAX(B2.ABCNTRL)   
         FROM CDCSANTANAMicroCredito..CBFIN B2 (NOLOCK)   
         WHERE B2.ABNROPER = B.ABNROPER and B2.ABNRGAR = B.ABNRGAR )  
  
   UPDATE  CARTEIRA_PERDA set ANO_MOD='1900' WHERE isnull(ANO_MOD,'') =''  
  
   UPDATE CARTEIRA_PERDA  
   SET  ATIVIDADE = TIPO + ' ' + FX_ANO.DESCR -- FAIXA  
   FROM CARTEIRA_PERDA (NOLOCK) LEFT JOIN   
     (SELECT * FROM FX_ANO_SAFRA (NOLOCK) WHERE  DT_DE = (SELECT MAX(A1.DT_DE)   
   ...
   SET  TIPO = CASE WHEN T.DESCR_TIPO= 'Automatico'  THEN 'Leves'   
        WHEN DESCR_TIPO= 'Leves'   THEN 'Leves'   
        WHEN DESCR_TIPO IN ('Utilitarios','Utilitario')  THEN 'Utilitario'   
        WHEN DESCR_TIPO= 'Motos'   THEN 'Motos'   
        WHEN T.DESCR_TIPO= 'Refinanciamento' THEN 'Leves'   
        WHEN T.DESCR_TIPO= 'Refinanciamento Utilitarios' THEN 'Utilitario'   
        WHEN T.DESCR_TIPO IS NULL THEN 'Leves'  
        ELSE T.DESCR_TIPO  END  
   FROM CDCSANTANAMicroCredito..COPER B (NOLOCK),  
     Ttipo_prod  T (NOLOCK),   
     TModa_tipo_prod M (NOLOCK)    
   WHERE 
   ...
   -- ajusta ANO VEICULO  -- ANO MODELO ********************  CARTEIRA_PERDA_RENE  
   UPDATE  CARTEIRA_PERDA_RENE set atividade=''  
   UPDATE  CARTEIRA_PERDA_RENE  
   SET  ANO_MOD =  ISNULL(ABANOMOD,'S/INF')  -- com Ano Modelo  
   FROM CDCSANTANAMicroCredito..CBFIN B (NOLOCK)  
   WHERE Contrato= B.ABNROPER  
   ...
   UPDATE  CARTEIRA_PERDA_RENE set ANO_MOD='1900' WHERE isnull(ANO_MOD,'') in (  '','S/INF') -- ALTERADO 3/2/2016  *******  
...
SELECT  DISTINCT  
  ...
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    (sum(isnull(VLR_PREJUIZO,0.0))/sum(FV_CONTRATO)*100.0) END  PERC_PREJUIZO_PERDA,  
       (SUM(VLR_DESCONTO)/1000)  DESCONTO,   
  SUM(PERC_DESCONTO)       PERC_DESCONTO,  
  CASE WHEN SUM(FV_CONTRATO) = 0.0 THEN 0.0 ELSE  
    (sum(isnull(VLR_DESCONTO,0.0))/sum(FV_CONTRATO)*100.0) END PERC_DESCONTO_PERDA,  
       (SUM(VLR_TOTAL_PERDA)/1000.0) PERDA_TOTAL,  
...
UPDATE TOTAL_SPREAD SET PERC_SAFRA   = CASE WHEN ISNULL(TOTAL_SAFRA,0.0) = 0.0 THEN 0.0 ELSE  
              (CONVERT (NUMERIC(12,5),(isnull(SAFRA,0.0) / TOTAL_SAFRA))*100.0)              
             END    WHERE TIPO = '05'         
...
FROM TOTAL_SPREAD (NOLOCK) where tipo IN ( '01','02' )  -- INCLUI TIPO 2 EM 7/2/17 PARA AJUSTAR O TTL GERAL = SOMA TIPO 1 SEM RENEG + 2 SOH RENEG  
...
FROM TOTAL_SPREAD (NOLOCK) where tipo in ('01')  
...
SELECT  DISTINCT  
  ...
  CASE WHEN SUM(SAFRA) = 0.0 THEN 0.0 ELSE  
   (((SUM (PREJUIZO)+ SUM(DESCONTO))+ SUM (PDD)) / SUM(SAFRA)*100.0)  END PERC_PERDA_RELIZADA,  
     '09'                                    TIPO  --- ' RENE - QUEBRA'  
FROM TOTAL_SPREAD (NOLOCK) WHERE TIPO = '05'  -- SOMA SOMENTE RENEG  
GROUP BY LEFT(ATIVIDADE,5)  
