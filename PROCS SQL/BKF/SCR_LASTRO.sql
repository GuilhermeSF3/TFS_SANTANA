...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_LASTRO' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_LASTRO]
GO
CREATE PROCEDURE [DBO].[SCR_LASTRO] (@DT_ATE AS SMALLDATETIME) AS  
  
SELECT --LEFT('1'+OPNROPER + REPLICATE(' ',17),17)+'999' + REPLICATE(' ',82) AS OPNROPER
DISTINCT OPNROPER
