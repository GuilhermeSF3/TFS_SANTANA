...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_POSICAO_VENCIMENTO' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_POSICAO_VENCIMENTO]
GO

CREATE Procedure [dbo].[SCR_POSICAO_VENCIMENTO] (@DT_ATE AS SMALLDATETIME) AS

SELECT OPDTBASE, PADTVCTO, OPDTBASE AS DT_INCL, OPNROPER, PANRPARC + '/' + CAST(OPQTDDPARC AS VARCHAR(10)) AS PANRPARC, CLCGC, CLNOMECLI, OPTXJRAP,PAVLRPR, 
PAVLRPR+PAVLRJR-CPSLDGRAP AS SALDO_DATA, PAVLRPR+PAVLRJR AS VLR_FINAL, CPSLDGMR AS MORA, CPSLDGMLT AS MULTA, CPSLDGIOC AS IOF, PAVLRPR+PAVLRJR+CPSLDGMR+CPSLDGMLT+CPSLDGIOC-CPSLDGRAP AS SLD_DEV 
...
OPDTBASE BETWEEN '1900-01-01' AND @DT_ATE --@DT_ATE
AND (PADTMOV IS NULL OR PADTMOV > @DT_ATE) --@DT_ATE 
AND  PADTVAL <= @DT_ATE --@DT_ATE
AND (OPDTLIQ IS NULL OR OPDTLIQ > @DT_ATE) --@DT_ATE
AND OPDTBASE <= @DT_ATE --@DT_ATE
...
AND S.PCDTMOV  >  @DT_ATE AND R.CCDTBASE <= @DT_ATE  --@dt_Ate
