...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_PASSO_ESTEIRA_CRED_ANALI' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_PASSO_ESTEIRA_CRED_ANALI] 
GO      
CREATE PROCEDURE SCR_PASSO_ESTEIRA_CRED_ANALI @DATADE SMALLDATETIME, @DATAATE SMALLDATETIME, @LOJA VARCHAR(10), @AGENTE VARCHAR(10),  
...
INSERT AUX_PASSO_ESTEIRA_CREDITO_ANALI (DTBASE,PPNRPROP)   
...
WHERE E.EPDTLNC BETWEEN @DATADE AND @DATAATE AND PPCODPRD NOT IN ('000221','000406','000515')  
AND E.EPNRDECI IN ('1549','100')  
--AND EPNRPROP NOT IN ('941931685') --PROPOSTAS COM EXCEÇÃO PARA EXIBIÇÃO NO RELATÓRIO.  
...
INNER JOIN PROPWEBSMC..CPROP (NOLOCK) ON E.EPNRPROP = PPNRPROP   
INNER JOIN PROPWEBSMC..CMOVP M(NOLOCK) ON PPNRPROP = MPNRPROP  
WHERE E.EPDTHRFIM BETWEEN @DATADE + '00:00:00' AND @DATAATE + '23:59:59' AND PPCODPRD NOT IN ('000221','000406','000515')  
AND E.EPNRDECI IN ('1549','100')  
--AND EPNRPROP NOT IN ('941931685') --PROPOSTAS COM EXCEÇÃO PARA EXIBIÇÃO NO RELATÓRIO.  
AND 1= 0+ CASE WHEN  @LOJA='99' THEN 1 -- TODAS AS LOJAS  
      WHEN  @LOJA<>'99' AND PPCODORG4 = @LOJA  THEN 1          
...
UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET   
...
DELETE FROM AUX_PASSO_ESTEIRA_CREDITO_ANALI WHERE DTBASE = CONVERT(CHAR(10),GETDATE(), 112) AND DT_SAI_LOJA IS NULL  
  
UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI 
...
WHERE PPNRPROP = E.EPNRPROP   
AND EPNRSEQ = (SELECT  MAX(E1.EPNRSEQ) FROM PROPWEBSMC..EPROP E1  (NOLOCK)    
       WHERE E1.EPNRPROP = E.EPNRPROP AND EPNRDECI IN ('2','91','96','18','79') AND E1.EPNRSEQ >= SEQ_MESA)  
--AND DTBASE BETWEEN @DATADE AND @DATAATE  
...
--INCLUSÃO 16/04/2019 PARA DASH DE TEMPO DE CRÉDITO
	...
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_ENT_ALÇA = @DT_HOJE WHERE DT_ENT_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_ENT_ALÇA = @HR_HOJE WHERE HR_ENT_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_SAI_ALÇA = @DT_HOJE WHERE DT_SAI_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_SAI_ALÇA = @HR_HOJE WHERE HR_SAI_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET TEMPO_ALÇA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ALÇA, HR_ENT_ALÇA,DT_SAI_ALÇA, HR_SAI_ALÇA))  

	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_ENT_ANALI = @DT_HOJE WHERE DT_ENT_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_ENT_ANALI = @HR_HOJE WHERE HR_ENT_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_SAI_ANALI = @DT_HOJE WHERE DT_SAI_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_SAI_ANALI = @HR_HOJE WHERE HR_SAI_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET TEMPO_ANALI = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ANALI, HR_ENT_ANALI,DT_SAI_ANALI, HR_SAI_ANALI))  

	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_ENT_MESA = @DT_HOJE WHERE DT_ENT_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_ENT_MESA = @HR_HOJE WHERE HR_ENT_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET DT_SAI_MESA = @DT_HOJE WHERE DT_SAI_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET HR_SAI_MESA = @HR_HOJE WHERE HR_SAI_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_CREDITO_ANALI SET TEMPO_MESA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_MESA, HR_ENT_MESA,DT_SAI_MESA, HR_SAI_MESA))  
END