...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_PASSO_ESTEIRA_FORMALIZ_DB' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_PASSO_ESTEIRA_FORMALIZ_DB] 
GO   

CREATE PROCEDURE SCR_PASSO_ESTEIRA_FORMALIZ_DB @DATADE SMALLDATETIME, @DATAATE SMALLDATETIME, @AGENTE VARCHAR(10), @PRODUTO VARCHAR(10),  
           @ANALISTA VARCHAR(20), @SITUAC VARCHAR(5), @VALOR_DE VARCHAR(100), @VALOR_ATE VARCHAR(100), @REANALISE VARCHAR(10)  
            AS    
  
DELETE FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB
    
INSERT AUX_PASSO_ESTEIRA_FORMALIZ_DB (DTBASE,PPNRPROP)    
...
UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET     
...
--DELETE FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE HR_ENT_PROM IS NULL AND DTBASE BETWEEN @DATADE AND @DATAATE  
  
--ATUALIZA TABELA TEMPORÁRIA COM TEMPO CALCULADO - PROMOTORA    
UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB    
...
--INCLUSÃO 16/04/2019 PARA DASH DE TEMPO DE CRÉDITO
	...
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_ENT_MESA = @DT_HOJE WHERE DT_ENT_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_ENT_MESA = @HR_HOJE WHERE HR_ENT_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_SAI_MESA = @DT_HOJE WHERE DT_SAI_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_SAI_MESA = @HR_HOJE WHERE HR_SAI_MESA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET TEMPO_MESA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_MESA, HR_ENT_MESA,DT_SAI_MESA, HR_SAI_MESA))  
		   
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_ENT_ANALI = @DT_HOJE WHERE DT_ENT_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_ENT_ANALI = @HR_HOJE WHERE HR_ENT_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_SAI_ANALI = @DT_HOJE WHERE DT_SAI_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_SAI_ANALI = @HR_HOJE WHERE HR_SAI_ANALI IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET TEMPO_ANALI = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ANALI, HR_ENT_ANALI,DT_SAI_ANALI, HR_SAI_ANALI))  
		   
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_ENT_ALÇA = @DT_HOJE WHERE DT_ENT_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_ENT_ALÇA = @HR_HOJE WHERE HR_ENT_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_SAI_ALÇA = @DT_HOJE WHERE DT_SAI_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_SAI_ALÇA = @HR_HOJE WHERE HR_SAI_ALÇA IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET TEMPO_ALÇA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ALÇA, HR_ENT_ALÇA,DT_SAI_ALÇA, HR_SAI_ALÇA)) 
	
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_ENT_REME = @DT_HOJE WHERE DT_ENT_REME IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_ENT_REME = @HR_HOJE WHERE HR_ENT_REME IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET DT_SAI_REME = @DT_HOJE WHERE DT_SAI_REME IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET HR_SAI_REME = @HR_HOJE WHERE HR_SAI_REME IS NULL
	UPDATE AUX_PASSO_ESTEIRA_FORMALIZ_DB SET TEMPO_REME = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_REME, HR_ENT_REME,DT_SAI_REME, HR_SAI_REME)) 	 
...
replace(cast(convert(decimal(10,2),cast(CAST((SUM(TEMPO_PROM)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int)+((CAST((SUM(TEMPO_PROM)/60.00)/COUNT(*) AS NUMERIC (10,2))-cast(CAST((SUM(TEMPO_PROM)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int))*.60)) as varchar),'
.',':') AS TEMPO_PROPOSTA      
FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE DTBASE BETWEEN @DATADE AND @DATAATE      
UNION        
SELECT       
'02' AS ORDEM,       
'FORMALIZAÇÃO' AS PROCESSO,       
...
replace(cast(convert(decimal(10,2),cast(CAST((SUM(TEMPO_MESA)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int)+((CAST((SUM(TEMPO_MESA)/60.00)/COUNT(*) AS NUMERIC (10,2))-cast(CAST((SUM(TEMPO_MESA)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int))*.60)) as varchar),'.
',':') AS TEMPO_PROPOSTA      
FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE DTBASE BETWEEN @DATADE AND @DATAATE      
UNION        
SELECT      
...
replace(cast(convert(decimal(10,2),cast(CAST((SUM(TEMPO_ANALI)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int)+((CAST((SUM(TEMPO_ANALI)/60.00)/COUNT(*) AS NUMERIC (10,2))-cast(CAST((SUM(TEMPO_ANALI)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int))*.60)) as varchar)
,'.  ',':') AS TEMPO_PROPOSTA      
FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE DTBASE BETWEEN @DATADE AND @DATAATE      
UNION        
SELECT      
...
replace(cast(convert(decimal(10,2),cast(CAST((SUM(TEMPO_ALÇA)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int)+((CAST((SUM(TEMPO_ALÇA)/60.00)/COUNT(*) AS NUMERIC (10,2))-cast(CAST((SUM(TEMPO_ALÇA)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int))*.60)) as varchar)  
,'.',':') AS TEMPO_PROPOSTA      
FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE DTBASE BETWEEN @DATADE AND @DATAATE      
UNION        
SELECT      
...
replace(cast(convert(decimal(10,2),cast(CAST((SUM(TEMPO_REME)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int)+((CAST((SUM(TEMPO_REME)/60.00)/COUNT(*) AS NUMERIC (10,2))-cast(CAST((SUM(TEMPO_REME)/60.00)/COUNT(*) AS NUMERIC (10,2)) as int))*.60)) as varchar),'.
',':') AS TEMPO_PROPOSTA      
FROM AUX_PASSO_ESTEIRA_FORMALIZ_DB WHERE DTBASE BETWEEN @DATADE AND @DATAATE      
UNION        
SELECT      
