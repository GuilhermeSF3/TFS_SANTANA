...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_PASSO_ESTEIRA_CRED' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_PASSO_ESTEIRA_CRED] 
GO      
CREATE PROCEDURE SCR_PASSO_ESTEIRA_CRED @DATADE SMALLDATETIME, @DATAATE SMALLDATETIME, @LOJA VARCHAR(10), @AGENTE VARCHAR(10),      
@PRODUTO VARCHAR(10), @ANALISTA VARCHAR(20), @SITUAC VARCHAR(5), @VALOR_DE VARCHAR(100), @VALOR_ATE VARCHAR(100), @REANALISE VARCHAR(10),      
       @ALCADA VARCHAR(2), @GERENCIA VARCHAR(5) AS      
      
-- TIAGO 30/07 - PROPOSTAS A PARTIR DA DATA QUE ENTROU NA MESA.      
INSERT AUX_PASSO_ESTEIRA_CREDITO (DTBASE,PPNRPROP)       
...
LEFT  JOIN DEPARA_GERENCIA (NOLOCK) ON COD_AGENTE = PPCODORG3
...
AND 1= 0+ CASE WHEN @ALCADA='99' THEN 1      
  ...
  ELSE 0 END   
AND 1= 0+ CASE WHEN  @GERENCIA='99' THEN 1 -- ANÁLISE/REANÁLISE      
      WHEN  @GERENCIA = '1' AND ISNULL(COD_GERENCIA,1) = 1 THEN 1      
   WHEN  @GERENCIA = '2' AND ISNULL(COD_GERENCIA,2) = 2 THEN 1             
      ELSE  0   END       
...
UPDATE AUX_PASSO_ESTEIRA_CREDITO SET       
...
AND EPNRSEQ = (SELECT  MIN(E1.EPNRSEQ) FROM PROPWEBSMC..EPROP E1  (NOLOCK)        
       WHERE E1.EPNRPROP = E.EPNRPROP AND EPNRDECI IN ('0'))      
...
DELETE FROM AUX_PASSO_ESTEIRA_CREDITO WHERE DTBASE = CONVERT(CHAR(10),GETDATE(), 112) AND DT_SAI_LOJA IS NULL      
      
UPDATE AUX_PASSO_ESTEIRA_CREDITO      
...
WHERE PPNRPROP = E.EPNRPROP      
AND EPNRSEQ = (SELECT  MAX(E1.EPNRSEQ) FROM PROPWEBSMC..EPROP E1  (NOLOCK)        
       WHERE E1.EPNRPROP = E.EPNRPROP AND EPNRDECI IN ('57') AND E1.EPNRSEQ > SEQ_ANALI)      
--AND DTBASE BETWEEN @DATADE AND @DATAATE      
...
--INCLUSÃO 16/04/2019 PARA DASH DE TEMPO DE CRÉDITO    
 ...
 --ATUALIZA TEMPO ALÇADA    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_ENT_ALÇA = @DT_HOJE WHERE DT_ENT_ALÇA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_ENT_ALÇA = @HR_HOJE WHERE HR_ENT_ALÇA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_SAI_ALÇA = @DT_HOJE WHERE DT_SAI_ALÇA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_SAI_ALÇA = @HR_HOJE WHERE HR_SAI_ALÇA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET TEMPO_ALÇA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ALÇA, HR_ENT_ALÇA,DT_SAI_ALÇA, HR_SAI_ALÇA))      
    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_ENT_ANALI = @DT_HOJE WHERE DT_ENT_ANALI IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_ENT_ANALI = @HR_HOJE WHERE HR_ENT_ANALI IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_SAI_ANALI = @DT_HOJE WHERE DT_SAI_ANALI IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_SAI_ANALI = @HR_HOJE WHERE HR_SAI_ANALI IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET TEMPO_ANALI = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_ANALI, HR_ENT_ANALI,DT_SAI_ANALI, HR_SAI_ANALI))      
    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_ENT_MESA = @DT_HOJE WHERE DT_ENT_MESA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_ENT_MESA = @HR_HOJE WHERE HR_ENT_MESA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET DT_SAI_MESA = @DT_HOJE WHERE DT_SAI_MESA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET HR_SAI_MESA = @HR_HOJE WHERE HR_SAI_MESA IS NULL    
 UPDATE AUX_PASSO_ESTEIRA_CREDITO SET TEMPO_MESA = (SIG.dbo.FN_TEMPO_ANALISE(DT_ENT_MESA, HR_ENT_MESA,DT_SAI_MESA, HR_SAI_MESA))   
END