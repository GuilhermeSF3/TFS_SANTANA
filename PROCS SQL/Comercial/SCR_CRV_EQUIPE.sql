...
IF EXISTS (SELECT name FROM sysobjects 
         WHERE name = 'SCR_CRV_EQUIPE' AND type = 'P')
   DROP PROCEDURE [dbo].[SCR_CRV_EQUIPE] 
GO  

CREATE Procedure [dbo].SCR_CRV_EQUIPE (            
 @DataContrato as datetime,            
@DT_REF SMALLDATETIME,                    
@DT_INI SMALLDATETIME,                
@DT_FIM SMALLDATETIME,               
@AGNT   INT,
@DTBX_DE SMALLDATETIME,
@DTBX_ATE SMALLDATETIME,
...
UPDATE CRV_PEND
SET VLR_OPER = 15
WHERE EPDTLNCBX BETWEEN @DTBX_DE AND @DTBX_ATE
...
CASE WHEN DIASATRASO <0 then '0' else  DIASATRASO  END diasAtraso, EPDTLNCBX, VLR_OPER
